...
public class PondFishSilhouette
{
  ...
  public void Draw(SpriteBatch b)
  {
    ...
    float rotation = num1 + (float) (Math.Sin((double) this._wiggleTimer + (double) this._randomOffset) * 2.0 * 3.1415927410125732 / 180.0);
    if ((double) this._velocity.Y < 0.0)
      rotation -= 0.174532935f;
    if ((double) this._velocity.Y > 0.0)
      rotation += 0.174532935f;
    if (this._flipped)
    {
    ...
    float num2 = Utility.Lerp(0.75f, 0.65f, Utility.Clamp(this._sinkAmount, 0.0f, 1f)) * Utility.Lerp(1f, 0.75f, (float) this._pond.currentOccupants.Value / 10f);
    ...
    b.Draw(dataOrErrorItem.GetTexture(), Game1.GlobalToLocal(Game1.viewport, position), new Rectangle?(dataOrErrorItem.GetSourceRect()), Color.Black * num3, rotation, origin, 4f * num2, effects, (float) ((double) this.position.Y / 10000.0 + 9.9999999747524271E-07));
  }
  ...
  public void Update(float time)
  {
    ...
    if ((double) this.nextDart <= 0.0 || (double) this.nextDart <= 0.5 && Game1.random.NextDouble() < 0.10000000149011612)
    {
      this.ResetDartTime();
    ...
    bool flag = (double) this._velocity.LengthSquared() > 0.0;
    if (flag)
    {
    ...
    float num3 = 1.3f;
    if ((double) this.position.X > ((double) (this._pond.tileX.Value + this._pond.tilesWide.Value) - (double) num3) * 64.0)
    {
      this.position.X = (float) (((double) (this._pond.tileX.Value + this._pond.tilesWide.Value) - (double) num3) * 64.0);
      this._velocity.X *= -1f;
      if (flag && (Game1.random.NextDouble() < 0.25 || (double) Math.Abs(this._velocity.X) > 30.0))
        this._flipped = !this._flipped;
    }
    if ((double) this.position.X < ((double) this._pond.tileX.Value + (double) num3) * 64.0)
    {
      this.position.X = (float) (((double) this._pond.tileX.Value + (double) num3) * 64.0);
      this._velocity.X *= -1f;
      if (flag && (Game1.random.NextDouble() < 0.25 || (double) Math.Abs(this._velocity.X) > 30.0))
        this._flipped = !this._flipped;
    }
    if ((double) this.position.Y > ((double) (this._pond.tileY.Value + this._pond.tilesHigh.Value) - (double) num3) * 64.0)
    {
      this.position.Y = (float) (((double) (this._pond.tileY.Value + this._pond.tilesHigh.Value) - (double) num3) * 64.0);
      this._velocity.Y *= -1f;
    }
    if ((double) this.position.Y >= ((double) this._pond.tileY.Value + (double) num3) * 64.0)
      return;
    this.position.Y = (float) (((double) this._pond.tileY.Value + (double) num3) * 64.0);
    this._velocity.Y *= -1f;
  }
}
