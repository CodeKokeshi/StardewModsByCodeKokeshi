...
public class DecorationFacade : SerializationCollectionFacade<int>
{
  ...
  public int this[int whichRoom]
  {
    get
    {
      ...
      int num;
      return !this.Field.TryGetValue(whichRoom, out num) ? 0 : num;
    }
    set
    {
      this.WarnDeprecation();
  ...
  public int Count => this.Field.Length == 0 ? 0 : this.Field.Keys.Max() + 1;

  public DecorationFacade()
  {
    NetIntDictionary<int, NetInt> netIntDictionary = new NetIntDictionary<int, NetInt>();
    netIntDictionary.InterpolationWait = false;
    this.Field = netIntDictionary;
    ...
    this.Field.OnValueAdded += (NetDictionary<int, int, NetInt, SerializableDictionary<int, int>, NetIntDictionary<int, NetInt>>.ContentsChangeEvent) ((whichRoom, which) =>
    {
      ...
      this.Field.FieldDict[whichRoom].fieldChangeEvent += (FieldChange<NetInt, int>) ((field, oldValue, newValue) => this.changed(whichRoom, newValue));
  ...
  {
    this.Field.Set((IEnumerable<KeyValuePair<int, int>>) other.Field.Pairs);
  }
