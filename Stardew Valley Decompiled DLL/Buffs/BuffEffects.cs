...
// Type: StardewValley.Buffs.BuffEffects
...
public class BuffEffects : INetObject<NetFields>
{
  ...
  public BuffEffects()
  {
    ...
    this.MultiplicativeFields = new NetFloat[6]
    {
    ...
    this.NetFields.SetOwner((INetObject<NetFields>) this).AddField((INetSerializable) this.CombatLevel, nameof (CombatLevel)).AddField((INetSerializable) this.FarmingLevel, nameof (FarmingLevel)).AddField((INetSerializable) this.FishingLevel, nameof (FishingLevel)).AddField((INetSerializable) this.MiningLevel, nameof (MiningLevel)).AddField((INetSerializable) this.LuckLevel, nameof (LuckLevel)).AddField((INetSerializable) this.ForagingLevel, nameof (ForagingLevel)).AddField((INetSerializable) this.MaxStamina, nameof (MaxStamina)).AddField((INetSerializable) this.MagneticRadius, nameof (MagneticRadius)).AddField((INetSerializable) this.Speed, nameof (Speed)).AddField((INetSerializable) this.Defense, nameof (Defense)).AddField((INetSerializable) this.Attack, nameof (Attack)).AddField((INetSerializable) this.AttackMultiplier, nameof (AttackMultiplier)).AddField((INetSerializable) this.Immunity, nameof (Immunity)).AddField((INetSerializable) this.KnockbackMultiplier, nameof (KnockbackMultiplier)).AddField((INetSerializable) this.WeaponSpeedMultiplier, nameof (WeaponSpeedMultiplier)).AddField((INetSerializable) this.CriticalChanceMultiplier, nameof (CriticalChanceMultiplier)).AddField((INetSerializable) this.CriticalPowerMultiplier, nameof (CriticalPowerMultiplier)).AddField((INetSerializable) this.WeaponPrecisionMultiplier, nameof (WeaponPrecisionMultiplier));
  }
  ...
  public void Add(BuffEffects other)
  {
    if (other == null)
      return;
    for (int index = 0; index < this.AdditiveFields.Length; ++index)
      this.AdditiveFields[index].Value += other.AdditiveFields[index].Value;
    for (int index = 0; index < this.MultiplicativeFields.Length; ++index)
      this.MultiplicativeFields[index].Value += other.MultiplicativeFields[index].Value;
  }
  ...
  {
    foreach (NetFieldBase<float, NetFloat> additiveField in this.AdditiveFields)
    {
      if ((double) additiveField.Value != 0.0)
        return true;
    }
    foreach (NetFieldBase<float, NetFloat> multiplicativeField in this.MultiplicativeFields)
    {
      if ((double) multiplicativeField.Value != 0.0)
        return true;
    }
    return false;
  }
  ...
  {
    foreach (NetFieldBase<float, NetFloat> additiveField in this.AdditiveFields)
      additiveField.Value = 0.0f;
    foreach (NetFieldBase<float, NetFloat> multiplicativeField in this.MultiplicativeFields)
      multiplicativeField.Value = 0.0f;
  }
