...
public class DiaryEvent : BaseFarmEvent
{
  ...
  public override bool setUp()
  {
    if (Game1.player.isMarriedOrRoommates())
      return true;
    foreach (string str1 in (NetHashSet<string>) Game1.player.mailReceived)
    {
      if (str1.Contains("diary"))
      {
        string str2 = str1.Split('_')[1];
        if (Game1.player.mailReceived.Add($"diary_{str2}_finished"))
        {
  ...
  /// <inheritdoc />
  public override bool tickUpdate(GameTime time) => !Game1.dialogueUp;
