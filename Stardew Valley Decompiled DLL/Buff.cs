...
public class Buff
{
    ...
    string description = null)
  {
    ...
    {
      this.displayName = TokenParser.ParseText(buffData.DisplayName);
      this.description = TokenParser.ParseText(buffData.Description);
      this.glow = Utility.StringToColor(buffData.GlowColor) ?? this.glow;
      this.millisecondsDuration = buffData.MaxDuration <= 0 || buffData.MaxDuration <= buffData.Duration ? buffData.Duration : Game1.random.Next(buffData.Duration, buffData.MaxDuration + 1);
      this.iconTexture = buffData.IconTexture == "TileSheets\\BuffsIcons" ? Game1.buffsIcons : Game1.content.Load<Texture2D>(buffData.IconTexture);
      this.iconSheetIndex = buffData.IconSpriteIndex;
      this.effects.Add(buffData.Effects);
      this.actionsOnApply = buffData.ActionsOnApply?.ToArray();
      flag = buffData.IsDebuff;
      this.customFields.TryAddMany<string, string>(buffData.CustomFields);
    }
    if (duration != -1)
    ...
    if ((isDebuff ?? flag) && Game1.player.isWearingRing("525") && this.millisecondsDuration != -2)
  ...
  public bool HasAnyEffects() => this.effects.HasAnyValue();

  /// <summary>Get a translated label showing the number of seconds remaining before the buff expires.</summary>
  public string getTimeLeft()
  {
    return $"{Game1.content.LoadString("Strings\\StringsFromCSFiles:Buff.cs.476")}{(this.millisecondsDuration / 60000).ToString()}:{(this.millisecondsDuration % 60000 / 10000).ToString()}{(this.millisecondsDuration % 60000 % 10000 / 1000).ToString()}";
  }
  ...
  public virtual bool update(GameTime time)
  {
    ...
    if (this.id == "13" && millisecondsDuration % 500 < this.millisecondsDuration % 500 && millisecondsDuration < 3000)
      Game1.multiplayer.broadcastSprites(Game1.player.currentLocation, new TemporaryAnimatedSprite(44, Game1.player.getStandingPosition() + new Vector2((float) (Game1.random.Next(-8, 12) - 40), (float) Game1.random.Next(-32, -16)), Color.Green * 0.5f, flipped: Game1.random.NextBool(), animationInterval: 70f)
      {
        scale = 1f
      });
  ...
  public virtual void OnAdded()
  {
    if (!(this.id == "19"))
      return;
    Game1.player.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(118, 227, 16 /*0x10*/, 13), Game1.player.getStandingPosition() + new Vector2(-32f, -21f), false, 0.0f, Color.White)
    {
