...
public class ForgeMenu : MenuWithInventory
{
  ...
  {
    ClickableTextureComponent textureComponent1 = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + 204, this.yPositionOnScreen + 212, 64 /*0x40*/, 64 /*0x40*/), this.forgeTextures, new Rectangle(142, 0, 16 /*0x10*/, 16 /*0x10*/), 4f);
    ...
    textureComponent1.item = this.leftIngredientSpot?.item;
    textureComponent1.fullyImmutable = true;
    this.leftIngredientSpot = textureComponent1;
    ClickableTextureComponent textureComponent2 = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + 348, this.yPositionOnScreen + 212, 64 /*0x40*/, 64 /*0x40*/), this.forgeTextures, new Rectangle(142, 0, 16 /*0x10*/, 16 /*0x10*/), 4f);
    ...
    textureComponent2.item = this.rightIngredientSpot?.item;
    textureComponent2.fullyImmutable = true;
    this.rightIngredientSpot = textureComponent2;
    ClickableTextureComponent textureComponent3 = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + 204, this.yPositionOnScreen + 308, 52, 56), this.forgeTextures, new Rectangle(0, 80 /*0x50*/, 13, 14), 4f);
    ...
    textureComponent3.item = this.startTailoringButton?.item;
    ...
    this.equipmentIcons.Add(new ClickableComponent(new Rectangle(0, 0, 64 /*0x40*/, 64 /*0x40*/), "Ring1")
    {
      myID = 110,
    ...
    this.equipmentIcons.Add(new ClickableComponent(new Rectangle(0, 0, 64 /*0x40*/, 64 /*0x40*/), "Ring2")
    {
      myID = 111,
      upNeighborID = -99998,
      downNeighborID = -99998,
...
label_13:
              ...
              break;
            case Tool tool:
              if (BaseEnchantment.GetAvailableEnchantmentsForItem(tool).Count > 0)
                goto label_13;
              goto default;
            default:
              num = BaseEnchantment.GetEnchantmentFromItem((Item) null, obj) != null ? 1 : 0;
              break;
          }
          bool flag = num != 0;
  ...
  {
    return item.canBeTrashed() || item is Tool tool && BaseEnchantment.GetAvailableEnchantmentsForItem(tool).Count > 0;
  }

  private void _rightIngredientSpotClicked()
  {
    Item obj = this.rightIngredientSpot.item;
    if (this.heldItem != null && !this.IsValidCraftIngredient(this.heldItem) || this.heldItem?.QualifiedItemId == "(O)848")
      return;
    Game1.playSound("stoneStep");
  ...
  public override void receiveLeftClick(int x, int y, bool playSound = true)
  {
      ...
      if (equipmentIcon.containsPoint(x, y))
      {
        switch (equipmentIcon.name)
        {
          case "Ring1":
            ...
            if (heldItem2 == Game1.player.leftRing.Value)
              return;
            switch (heldItem2)
            {
              case null:
              case Ring _:
                this.heldItem = (Item) Game1.player.Equip<Ring>(heldItem2 as Ring, Game1.player.leftRing);
                if (Game1.player.leftRing.Value != null)
          ...
          case "Ring2":
            if (!this.HighlightItems((Item) Game1.player.rightRing.Value) && Game1.player.rightRing.Value != null)
              return;
            Item heldItem3 = this.heldItem;
            if (heldItem3 == Game1.player.rightRing.Value)
              return;
            switch (heldItem3)
            {
              case null:
              case Ring _:
                this.heldItem = (Item) Game1.player.Equip<Ring>(heldItem3 as Ring, Game1.player.rightRing);
                if (Game1.player.rightRing.Value != null)
                  Game1.playSound("crit");
                else if (this.heldItem != null)
                  Game1.playSound("dwop");
    ...
    else if (this.startTailoringButton.containsPoint(x, y))
    {
      if (this.heldItem == null)
      {
        ...
        if (!flag && this.IsValidCraft(this.leftIngredientSpot.item, this.rightIngredientSpot.item) && Game1.player.Items.ContainsId("(O)848", this.GetForgeCost(this.leftIngredientSpot.item, this.rightIngredientSpot.item)))
        {
          Game1.playSound("bigSelect");
          ...
          if (this.rightIngredientSpot.item?.QualifiedItemId == "(O)74")
          {
            ...
            bounds.Offset(-32, -32);
            TemporaryAnimatedSpriteList values = Utility.sparkleWithinArea(bounds, 6, Color.White, 80 /*0x50*/, 1600);
            values[0].startSound = "discoverMineral";
            this.tempSprites.AddRange((IEnumerable<TemporaryAnimatedSprite>) values);
            bounds = this.rightIngredientSpot.bounds;
            bounds.Inflate(-16, -16);
            int num = 30;
            for (int index = 0; index < num; ++index)
              this.tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(114, 48 /*0x30*/, 2, 2), Utility.getRandomPositionInThisRectangle(bounds, Game1.random), false, 0.0f, Color.White)
              {
    ...
    else if (this.unforgeButton.containsPoint(x, y))
    {
      if (this.rightIngredientSpot.item == null)
      {
        ...
        {
          if (this.leftIngredientSpot.item is MeleeWeapon meleeWeapon && !Game1.player.couldInventoryAcceptThisItem("(O)848", meleeWeapon.GetTotalForgeLevels() * 5 + (meleeWeapon.GetTotalForgeLevels() - 1) * 2))
          {
  ...
  public virtual int GetForgeCost(Item left_item, Item right_item)
  {
    switch (right_item?.QualifiedItemId)
    {
      ...
      default:
        ...
        {
          case Tool tool:
            return !(tool is MeleeWeapon) || !(right_item is MeleeWeapon) ? this.GetForgeCostAtLevel(tool.GetTotalForgeLevels()) : 10;
          case Ring _:
            return !(right_item is Ring) ? 1 : 20;
          default:
            return 1;
        }
    }
    ...
    else
    {
      switch (this._craftState)
      {
        ...
        case ForgeMenu.CraftState.MissingShards:
          this.displayedDescription = this.heldItem?.QualifiedItemId == "(O)848" ? Game1.content.LoadString("Strings\\UI:Forge_shards") : Game1.content.LoadString("Strings\\UI:Forge_notenoughshards");
          break;
  ...
  {
    return left_item != null && right_item != null && (left_item is Tool tool && tool.CanForge(right_item) || left_item is Ring ring1 && right_item is Ring ring2 && ring1.CanCombine(ring2));
  }
  ...
  public virtual Item CraftItem(Item left_item, Item right_item, bool forReal = false)
  {
    ...
    switch (left_item)
    {
      case Tool tool when !tool.Forge(right_item, forReal):
        return (Item) null;
      case Ring ring2 when right_item is Ring ring1:
        left_item = (Item) ring2.Combine(ring1);
        break;
    }
    return left_item;
  }
      ...
      switch (this.leftIngredientSpot.item)
      {
        case MeleeWeapon meleeWeapon:
          ...
          if (meleeWeapon.appearance.Value != null)
          {
            ...
            meleeWeapon.appearance.Value = (string) null;
          ...
          Utility.CollectOrDrop(ItemRegistry.Create("(O)848", num / 2));
          break;
        case CombinedRing combinedRing:
  ...
  {
    return (ignore_right_slot_occupancy || this.rightIngredientSpot.item == null) && (this.leftIngredientSpot.item is MeleeWeapon meleeWeapon && (meleeWeapon.GetTotalForgeLevels() > 0 || meleeWeapon.appearance.Value != null) || this.leftIngredientSpot.item is CombinedRing);
  }
  ...
  public override void draw(SpriteBatch b)
  {
    ...
    Game1.DrawBox(this.xPositionOnScreen - 64 /*0x40*/, this.yPositionOnScreen + 128 /*0x80*/, 128 /*0x80*/, 201, new Color?(new Color(116, 11, 3)));
    ...
    b.Draw(this.forgeTextures, new Vector2((float) (this.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2 - 4), (float) (this.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder)), new Rectangle?(new Rectangle(0, 0, 142, 80 /*0x50*/)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.87f);
    Color color = Color.White;
    if (this._craftState == ForgeMenu.CraftState.MissingShards)
      color = Color.Gray * 0.75f;
    b.Draw(this.forgeTextures, new Vector2((float) (this.xPositionOnScreen + 276), (float) (this.yPositionOnScreen + 300)), new Rectangle?(new Rectangle(142, 16 /*0x10*/, 17, 17)), color, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.1f);
    ...
    foreach (ClickableComponent equipmentIcon in this.equipmentIcons)
    {
      switch (equipmentIcon.name)
      {
        case "Ring1":
          if (Game1.player.leftRing.Value != null)
          {
            b.Draw(this.forgeTextures, equipmentIcon.bounds, new Rectangle?(new Rectangle(0, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White);
            float transparency = 1f;
            if (!this.HighlightItems((Item) Game1.player.leftRing.Value))
              transparency = 0.5f;
          ...
          b.Draw(this.forgeTextures, equipmentIcon.bounds, new Rectangle?(new Rectangle(16 /*0x10*/, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White);
          continue;
        case "Ring2":
          if (Game1.player.rightRing.Value != null)
          {
            b.Draw(this.forgeTextures, equipmentIcon.bounds, new Rectangle?(new Rectangle(0, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White);
            float transparency = 1f;
            if (!this.HighlightItems((Item) Game1.player.rightRing.Value))
              transparency = 0.5f;
    ...
    {
      if (this.hoveredItem == this.craftResultDisplay.item && this.rightIngredientSpot.item?.QualifiedItemId == "(O)74")
        BaseEnchantment.hideEnchantmentName = true;
      else if (this.hoveredItem == this.craftResultDisplay.item && this.rightIngredientSpot.item?.QualifiedItemId == "(O)852")
