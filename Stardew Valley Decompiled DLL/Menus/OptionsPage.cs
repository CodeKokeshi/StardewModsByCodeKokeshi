...
public class OptionsPage : IClickableMenu
{
  ...
  {
    this.upArrow = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + width + 16 /*0x10*/, this.yPositionOnScreen + 64 /*0x40*/, 44, 48 /*0x30*/), Game1.mouseCursors, new Rectangle(421, 459, 11, 12), 4f);
    this.downArrow = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + width + 16 /*0x10*/, this.yPositionOnScreen + height - 64 /*0x40*/, 44, 48 /*0x30*/), Game1.mouseCursors, new Rectangle(421, 472, 11, 12), 4f);
    ...
    this.scrollBarRunner = new Rectangle(this.scrollBar.bounds.X, this.upArrow.bounds.Y + this.upArrow.bounds.Height + 4, this.scrollBar.bounds.Width, height - 128 /*0x80*/ - this.upArrow.bounds.Height - 8);
    for (int index1 = 0; index1 < 7; ++index1)
      this.optionSlots.Add(new ClickableComponent(new Rectangle(this.xPositionOnScreen + 16 /*0x10*/, this.yPositionOnScreen + 80 /*0x50*/ + 4 + index1 * ((height - 128 /*0x80*/) / 7) + 16 /*0x10*/, width - 32 /*0x20*/, (height - 128 /*0x80*/) / 7 + 4), index1.ToString() ?? "")
      {
        myID = index1,
    ...
    {
      this.options.Add((OptionsElement) new OptionsDropDown(Game1.content.LoadString("Strings\\UI:GameMenu_ServerMode"), 31 /*0x1F*/));
      this.options.Add((OptionsElement) new OptionsCheckbox(Game1.content.LoadString("Strings\\UI:OptionsPage_IPConnections"), 30));
      this.options.Add((OptionsElement) new OptionsCheckbox(Game1.content.LoadString("Strings\\UI:OptionsPage_FarmhandCreation"), 32 /*0x20*/));
    }
    ...
    {
      this.options.Add((OptionsElement) new OptionsButton(Game1.content.LoadString("Strings\\UI:GameMenu_ServerInvite"), new Action(this.offerInvite)));
      if (Program.sdk.Networking.SupportsInviteCodes())
        this.options.Add((OptionsElement) new OptionsButton(Game1.content.LoadString("Strings\\UI:OptionsPage_ShowInviteCode"), new Action(this.showInviteCode)));
    }
    if (flag)
    ...
    this.options.Add((OptionsElement) new OptionsDropDown(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11252"), 9));
    if (Game1.game1.IsMainInstance)
    ...
    {
      this.options.Add((OptionsElement) new OptionsCheckbox("使用平滑字体", 44));
      this.options.Add((OptionsElement) new OptionsSlider("对话字体大小", 45));
    }
    else if (Game1.content.GetCurrentLanguage() == LocalizedContentManager.LanguageCode.ru)
      this.options.Add((OptionsElement) new OptionsCheckbox("Использовать альтернативный шрифт", 46));
    this.options.Add(new OptionsElement(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11274")));
    this.options.Add((OptionsElement) new OptionsCheckbox(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11275"), 16 /*0x10*/));
    ...
    if (Game1.game1.IsMainInstance)
    {
      ...
      this.options.Add((OptionsElement) new OptionsInputListener(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11288"), 16 /*0x10*/, this.optionSlots[0].bounds.Width));
      this.options.Add((OptionsElement) new OptionsInputListener(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.toolbarSwap"), 32 /*0x20*/, this.optionSlots[0].bounds.Width));
      ...
      this.options.Add((OptionsElement) new OptionsInputListener(Game1.content.LoadString("Strings\\StringsFromCSFiles:OptionsPage.cs.11300"), 31 /*0x1F*/, this.optionSlots[0].bounds.Width));
    }
    ...
    {
      OptionsButton optionsButton = new OptionsButton(Game1.content.LoadString("Strings\\UI:OptionsPage_ScreenshotHeader").Replace(":", ""), new Action(TakeScreenshot));
    ...
    else
    {
      ...
      Rectangle buttonRect = new Rectangle(72, 31 /*0x1F*/, 18, 16 /*0x10*/);
    ...
    if (!Game1.game1.CanBrowseScreenshots())
      return;
    this.options.Add((OptionsElement) new OptionsButton(Game1.content.LoadString("Strings\\UI:OptionsPage_OpenFolder"), new Action(Game1.game1.BrowseScreenshots)));
    ...
    void TakeScreenshot()
    {
      ...
      Game1.flashAlpha = 1f;
      e.greyedOut = true;
      string mapScreenshot = Game1.game1.takeMapScreenshot(new float?(), (string) null, new Action(OnDone));
      if (mapScreenshot != null)
        Game1.addHUDMessage(new HUDMessage(mapScreenshot, 6));
      Game1.playSound("cameraNoise");

      void OnDone() => e.greyedOut = false;
    }
  }
  ...
  private void waitForServerConnection(Action onConnection)
  {
    ...
    else
    {
      ...
      Game1.activeClickableMenu = (IClickableMenu) new ServerConnectionDialog(new ConfirmationDialog.behavior(OnConfirm), new ConfirmationDialog.behavior(OnClose));
  ...
  private void offerInvite() => this.waitForServerConnection(new Action(Game1.server.offerInvite));

  private void showInviteCode()
  {
    IClickableMenu thisMenu = Game1.activeClickableMenu;
    this.waitForServerConnection((Action) (() => Game1.activeClickableMenu = (IClickableMenu) new InviteCodeDialog(Game1.server.getInviteCode(), new ConfirmationDialog.behavior(OnClose))));
  ...
  protected override void customSnapBehavior(int direction, int oldRegion, int oldID)
  {
    ...
    else
    {
      ...
      {
        this.currentlySnappedComponent = this.getComponentWithID(12348);
  ...
  public void postWindowSizeChange(IClickableMenu oldPage)
  {
    if (!(oldPage is OptionsPage optionsPage))
      return;
    ClickableComponent snappedComponent = optionsPage.getCurrentlySnappedComponent();
    int id = snappedComponent != null ? snappedComponent.myID : -1;
    int currentItemIndex = optionsPage.currentItemIndex;
    if (Game1.options.SnappyMenus)
