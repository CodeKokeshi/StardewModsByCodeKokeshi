...
public class ClickableComponent : IScreenReadable
{
  ...
  {
    if (!this.visible || !new Rectangle(this.bounds.X - extraMargin, this.bounds.Y - extraMargin, this.bounds.Width + extraMargin * 2, this.bounds.Height + extraMargin * 2).Contains(x, y))
      return false;
    Game1.SetFreeCursorDrag();
    return true;
  }
