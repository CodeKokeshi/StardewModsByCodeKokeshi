...
public class EmoteSelector : IClickableMenu
{
  ...
  public EmoteSelector(int emote_index, string selected_emote = "")
    : base(Game1.uiViewport.Width / 2 - (800 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2 - 64 /*0x40*/, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2 + 64 /*0x40*/)
  {
    ...
    for (int emote_index1 = 0; emote_index1 < Farmer.EMOTES.Length; ++emote_index1)
    {
      ...
      {
        ClickableTextureComponent textureComponent1 = new ClickableTextureComponent(new Rectangle(0, 0, 80 /*0x50*/, 68), this.emoteTexture, EmoteMenu.GetEmoteNonBubbleSpriteRect(emote_index1), 4f, true);
  ...
  {
    this.scrollView = new Rectangle(this.xPositionOnScreen + 64 /*0x40*/, this.yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder - 4, this.width - 128 /*0x80*/, this.height - IClickableMenu.borderWidth - IClickableMenu.spaceToClearTopBorder - 64 /*0x40*/ + 8);
  ...
  {
    IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256 /*0x0100*/, 60, 60), this.xPositionOnScreen - 128 /*0x80*/ - 8, this.yPositionOnScreen + 128 /*0x80*/ - 8, 192 /*0xC0*/, 164, Color.White, drawShadow: false);
    ...
    foreach (ClickableTextureComponent emoteButton in this.emoteButtons)
    {
      ...
      {
        IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(64 /*0x40*/, 320, 60, 60), emoteButton.bounds.X + 64 /*0x40*/ + 8, emoteButton.bounds.Y + 8, this.scrollView.Width / 3 - 64 /*0x40*/ - 16 /*0x10*/, emoteButton.bounds.Height - 16 /*0x10*/, Color.White, drawShadow: false);
      ...
      if (emoteButton != this._selectedEmote && emoteButton.hoverText != "" && Game1.currentGameTime.TotalGameTime.Milliseconds % 500 < 250)
        b.Draw(emoteButton.texture, emoteButton.getVector2(), new Rectangle?(new Rectangle(emoteButton.sourceRect.X + 80 /*0x50*/, emoteButton.sourceRect.Y, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
    }
    if (this._selectedEmote != null)
    {
      for (int index = 0; index < 8; ++index)
      {
        float num = Utility.Lerp(0.0f, 6.28318548f, (float) index / 8f);
        Vector2 zero = Vector2.Zero with
        {
          X = (float) (int) ((double) (this.xPositionOnScreen - 64 /*0x40*/ + (int) (Math.Cos((double) num) * 12.0) * 4) - 3.5),
          Y = (float) (int) ((double) (this.yPositionOnScreen + 192 /*0xC0*/ + (int) (-Math.Sin((double) num) * 12.0) * 4) - 3.5)
        };
        Utility.drawWithShadow(b, this.emoteTexture, zero, new Rectangle(64 /*0x40*/ + (index == this.emoteIndex ? 8 : 0), 48 /*0x30*/, 8, 8), Color.White, 0.0f, Vector2.Zero);
      }
    }
