...
public class DialogueBox : IClickableMenu
{
  ...
  public DialogueBox(string dialogue)
  {
    ...
    this.width = Math.Min(1240, SpriteText.getWidthOfString(this.dialogues[0]) + 64 /*0x40*/);
    ...
    this.y = Game1.uiViewport.Height - this.height - 64 /*0x40*/;
    this.setUpIcons();
  }
  ...
  public DialogueBox(Dialogue dialogue)
  {
    ...
    this.x = (int) Utility.getTopLeftPositionForCenteringOnScreen(this.width, this.height).X;
    this.y = Game1.uiViewport.Height - this.height - 64 /*0x40*/;
    this.friendshipJewel = new Rectangle(this.x + this.width - 64 /*0x40*/, this.y + 256 /*0x0100*/, 44, 44);
  ...
  public DialogueBox(List<string> dialogues)
  {
    ...
    this.width = Math.Min(1200, SpriteText.getWidthOfString(dialogues[0]) + 64 /*0x40*/);
    this.height = SpriteText.getHeightOfString(dialogues[0], this.width - 16 /*0x10*/);
    this.x = (int) Utility.getTopLeftPositionForCenteringOnScreen(this.width, this.height).X;
    this.y = Game1.uiViewport.Height - this.height - 64 /*0x40*/;
    this.setUpIcons();
  }
  ...
  public override void receiveKeyPress(Keys key)
  {
    ...
    else if (this.isQuestion && !Game1.eventUp && this.characterDialogue == null)
    {
      ...
      {
        Response[] responses = this.responses;
        if ((responses != null ? (responses.Length != 0 ? 1 : 0) : 0) != 0 && Game1.currentLocation.answerDialogue(this.responses[this.responses.Length - 1]))
          Game1.playSound("smallSelect");
        this.selectedResponse = -1;
        this.tryOutro();
      }
      ...
      else
      {
        if (key != Keys.Y)
          return;
        Response[] responses = this.responses;
        if ((responses != null ? (responses.Length != 0 ? 1 : 0) : 0) == 0 || !this.responses[0].responseKey.Equals("Yes") || !Game1.currentLocation.answerDialogue(this.responses[0]))
          return;
        Game1.playSound("smallSelect");
        this.selectedResponse = -1;
        this.tryOutro();
  ...
  public override void receiveLeftClick(int x, int y, bool playSound = true)
  {
    ...
    else
    {
      ...
      else if (this.characterDialogue == null)
      {
        ...
        {
          this.width = Math.Min(1200, SpriteText.getWidthOfString(this.dialogues[0]) + 64 /*0x40*/);
          this.height = SpriteText.getHeightOfString(this.dialogues[0], this.width - 16 /*0x10*/);
          this.x = (int) Utility.getTopLeftPositionForCenteringOnScreen(this.width, this.height).X;
          this.y = Game1.uiViewport.Height - this.height - 64 /*0x40*/;
  ...
  private void setUpIcons()
  {
    ...
    else
    {
      ...
      if ((dialogues != null ? (__nonvirtual (dialogues.Count) > 1 ? 1 : 0) : 0) != 0)
        this.setUpNextPageIcon();
      else
  ...
  public override void performHoverAction(int mouseX, int mouseY)
  {
    ...
    if (!this.transitioning && this.characterIndexInDialogue >= this.getCurrentString().Length - 1)
    {
      ...
      if (this.isQuestion)
      {
        ...
        int num1 = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), this.width - 16 /*0x10*/) + 48 /*0x30*/;
        int num2 = 8;
        for (int index = 0; index < this.responses.Length; ++index)
        {
          if (mouseY >= num1 - num2 && mouseY < num1 + SpriteText.getHeightOfString(this.responses[index].responseText, this.width - 16 /*0x10*/) + num2)
          {
            this.selectedResponse = index;
            int num3 = index;
            int? count = this.responseCC?.Count;
            int valueOrDefault = count.GetValueOrDefault();
            if (num3 < valueOrDefault & count.HasValue)
            {
          ...
          num1 += SpriteText.getHeightOfString(this.responses[index].responseText, this.width - 16 /*0x10*/) + 16 /*0x10*/;
        }
  ...
  {
    return this.width >= 642 && !Game1.eventUp && !this.isQuestion && this.isPortraitBox() && !this.friendshipJewel.Equals(Rectangle.Empty) && this.characterDialogue?.speaker != null && Game1.player.friendshipData.ContainsKey(this.characterDialogue.speaker.Name) && this.characterDialogue.speaker.Name != "Henchman";
  }
  ...
  private void checkDialogue(Dialogue d)
  {
    ...
    if (this.characterDialoguesBrokenUp.Count == 1)
      str1 = SpriteText.getSubstringBeyondHeight(this.characterDialoguesBrokenUp.Peek(), this.width - 460 - 20, this.height - 16 /*0x10*/);
    if (str1.Length > 0)
    {
  ...
  {
    int widthConstraint = this.width - 16 /*0x10*/;
    this.heightForQuestions = SpriteText.getHeightOfString(this.getCurrentString(), widthConstraint);
    foreach (Response response in this.responses)
      this.heightForQuestions += SpriteText.getHeightOfString(response.responseText, widthConstraint) + 16 /*0x10*/;
    this.heightForQuestions += 40;
  }

  public bool isPortraitBox()
  {
    return this.characterDialogue?.speaker?.Portrait != null && this.characterDialogue.showPortrait && Game1.options.showPortraits;
  }
  ...
  public void drawBox(SpriteBatch b, int xPos, int yPos, int boxWidth, int boxHeight)
  {
    if (!this.transitionInitialized)
      return;
    b.Draw(Game1.mouseCursors, new Rectangle(xPos, yPos, boxWidth, boxHeight), new Rectangle?(new Rectangle(306, 320, 16 /*0x10*/, 16 /*0x10*/)), Color.White);
    b.Draw(Game1.mouseCursors, new Rectangle(xPos, yPos - 20, boxWidth, 24), new Rectangle?(new Rectangle(275, 313, 1, 6)), Color.White);
    b.Draw(Game1.mouseCursors, new Rectangle(xPos + 12, yPos + boxHeight, boxWidth - 20, 32 /*0x20*/), new Rectangle?(new Rectangle(275, 328, 1, 8)), Color.White);
    b.Draw(Game1.mouseCursors, new Rectangle(xPos - 32 /*0x20*/, yPos + 24, 32 /*0x20*/, boxHeight - 28), new Rectangle?(new Rectangle(264, 325, 8, 1)), Color.White);
  ...
  private bool shouldPortraitShake(Dialogue d)
  {
    ...
    // ISSUE: explicit non-virtual call
    return shakePortraits != null && __nonvirtual (shakePortraits.Count) > 0 && shakePortraits.Contains(d.getPortraitIndex());
  }

  public void drawPortrait(SpriteBatch b)
  {
    NPC npc = this.characterDialogue.speaker;
    if (!Game1.IsMasterGame && !npc.EventActor)
    {
      GameLocation currentLocation = npc.currentLocation;
      if ((currentLocation != null ? (currentLocation.IsActiveLocation() ? 1 : 0) : 0) == 0)
      {
    ...
    Texture2D texture2D = this.characterDialogue.overridePortrait ?? npc.Portrait;
    Rectangle rectangle = Game1.getSourceRectForStandardTileSheet(texture2D, this.characterDialogue.getPortraitIndex(), 64 /*0x40*/, 64 /*0x40*/);
    if (!texture2D.Bounds.Contains(rectangle))
      rectangle = new Rectangle(0, 0, 64 /*0x40*/, 64 /*0x40*/);
    int num5 = this.shouldPortraitShake(this.characterDialogue) ? Game1.random.Next(-1, 2) : 0;
    b.Draw(texture2D, new Vector2((float) (num3 + 16 /*0x10*/ + num5), (float) (num4 + 24)), new Rectangle?(rectangle), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
    SpriteText.drawStringHorizontallyCenteredAt(b, npc.getName(), num1 + num2 / 2, num4 + 296 + 16 /*0x10*/);
    if (!this.shouldDrawFriendshipJewel())
      return;
    b.Draw(Game1.mouseCursors, new Vector2((float) this.friendshipJewel.X, (float) this.friendshipJewel.Y), new Rectangle?(Game1.player.getFriendshipHeartLevelForNPC(npc.Name) >= 10 ? new Rectangle(269, 494, 11, 11) : new Rectangle(Math.Max(140, 140 + (int) (Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 1000.0 / 250.0) * 11), Math.Max(532, 532 + Game1.player.getFriendshipHeartLevelForNPC(npc.Name) / 2 * 11), 11, 11)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
  }
    ...
    if (this.isQuestion && this.characterIndexInDialogue >= this.getCurrentString().Length - 1 && !this.transitioning)
    {
      ...
      if (!this._showedOptions)
      {
        ...
        if (this.responses != null)
        {
          ...
          int y = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), this.width) + 48 /*0x30*/;
          for (int index = 0; index < this.responses.Length; ++index)
          {
            this.responseCC.Add(new ClickableComponent(new Rectangle(this.x + 8, y, this.width - 8, SpriteText.getHeightOfString(this.responses[index].responseText, this.width) + 16 /*0x10*/), "")
            {
              myID = index,
              downNeighborID = index < this.responses.Length - 1 ? index + 1 : -1,
              upNeighborID = index > 0 ? index - 1 : -1
            });
            y += SpriteText.getHeightOfString(this.responses[index].responseText, this.width) + 16 /*0x10*/;
          }
        }
  ...
  {
    if (this.width < 16 /*0x10*/ || this.height < 16 /*0x10*/)
      return;
    if (this.transitioning)
    {
    ...
    else
    {
      if (this.isQuestion)
      {
        ...
        b.Draw(Game1.mouseCursors_1_6, new Vector2((float) (this.x + this.width - 72), (float) (this.y - (this.heightForQuestions - this.height) - 88)), new Rectangle?(new Rectangle(495, 461, 17, 19)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.87f);
        b.Draw(Game1.mouseCursors_1_6, new Vector2((float) (this.x + this.width - 52), (float) (this.y - (this.heightForQuestions - this.height) - 88 + 16 /*0x10*/)), new Rectangle?(new Rectangle(470 + (int) Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 900 / 150 * 7, 447, 7, 12)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
        SpriteText.drawString(b, this.getCurrentString(), this.x + 8, this.y + 12 - (this.heightForQuestions - this.height), this.characterIndexInDialogue, this.width - 16 /*0x10*/);
        if (this.characterIndexInDialogue >= this.getCurrentString().Length - 1)
        {
          int y = this.y - (this.heightForQuestions - this.height) + SpriteText.getHeightOfString(this.getCurrentString(), this.width - 16 /*0x10*/) + 48 /*0x30*/;
          for (int index = 0; index < this.responses.Length; ++index)
          {
            if (index == this.selectedResponse)
              IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), this.x + 4, y - 8, this.width - 8, SpriteText.getHeightOfString(this.responses[index].responseText, this.width) + 16 /*0x10*/, Color.White, 4f, false);
      ...
      {
        this.drawBox(b, this.x + this.width / 2 - (int) ((double) (this.aboveDialogueImage.sourceRect.Width / 2) * (double) this.aboveDialogueImage.scale), this.y - 64 /*0x40*/ - 4 - (int) ((double) this.aboveDialogueImage.sourceRect.Height * (double) this.aboveDialogueImage.scale), (int) ((double) this.aboveDialogueImage.sourceRect.Width * (double) this.aboveDialogueImage.scale), (int) ((double) this.aboveDialogueImage.sourceRect.Height * (double) this.aboveDialogueImage.scale) + 8);
        Utility.drawWithShadow(b, this.aboveDialogueImage.texture, new Vector2((float) (this.x + this.width / 2) - (float) (this.aboveDialogueImage.sourceRect.Width / 2) * this.aboveDialogueImage.scale, (float) (this.y - 64 /*0x40*/ - (int) ((double) this.aboveDialogueImage.sourceRect.Height * (double) this.aboveDialogueImage.scale))), this.aboveDialogueImage.sourceRect, Color.White, 0.0f, Vector2.Zero, this.aboveDialogueImage.scale, layerDepth: 1f);
      }
      if (this.hoverText.Length > 0)
        SpriteText.drawStringWithScrollBackground(b, this.hoverText, this.friendshipJewel.Center.X - SpriteText.getWidthOfString(this.hoverText) / 2, this.friendshipJewel.Y - 64 /*0x40*/);
      this.drawMouse(b);
    }
  }
}
