      ...
      if ((double) this.power >= 99.0)
      {
        if ((double) this.endTimer < 1500.0)
        {
          ...
          if (!this.showedResult && Game1.random.NextDouble() < 0.08)
            Game1.multiplayer.broadcastSprites(Game1.currentLocation, new TemporaryAnimatedSprite(10 + Game1.random.Next(2), new Vector2(31f, 55f) * 64f + new Vector2((float) Game1.random.Next(-64, 64 /*0x40*/), (float) Game1.random.Next(-64, 64 /*0x40*/)), Color.Yellow)
            {
              layerDepth = 1f
            });
        }
