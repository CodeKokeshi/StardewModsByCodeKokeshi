...
public class InventoryMenu : IClickableMenu
{
  ...
  public InventoryMenu(
    ...
    int verticalGap = 0,
    bool drawSlots = true)
    : base(xPosition, yPosition, 64 /*0x40*/ * ((capacity == -1 ? 36 : capacity) / rows), 64 /*0x40*/ * rows + 16 /*0x10*/)
  {
    ...
    for (int index = 0; index < this.capacity; ++index)
    {
      ...
      this.inventory.Add(new ClickableComponent(new Rectangle(xPosition + index % (this.capacity / rows) * 64 /*0x40*/ + horizontalGap * (index % (this.capacity / rows)), this.yPositionOnScreen + index / (this.capacity / rows) * (64 /*0x40*/ + verticalGap) + (index / (this.capacity / rows) - 1) * 4 - (index > this.capacity / rows || !playerInventory || verticalGap != 0 ? 0 : 12), 64 /*0x40*/, 64 /*0x40*/), index.ToString() ?? "")
      {
        myID = index,
    ...
    this.dropItemInvisibleButton = new ClickableComponent(new Rectangle(xPosition - IClickableMenu.borderWidth - IClickableMenu.spaceToClearSideBorder - 128 /*0x80*/, this.yPositionOnScreen - 12, 64 /*0x40*/, 64 /*0x40*/), "")
    {
...
label_15:
  ...
  public override void draw(SpriteBatch b, int red, int green, int blue)
  {
    ...
    if (this.drawSlots)
    {
      for (int index = 0; index < this.capacity; ++index)
      {
        Vector2 position = new Vector2((float) (this.xPositionOnScreen + index % (this.capacity / this.rows) * 64 /*0x40*/ + this.horizontalGap * (index % (this.capacity / this.rows))), (float) (this.yPositionOnScreen + index / (this.capacity / this.rows) * (64 /*0x40*/ + this.verticalGap) + (index / (this.capacity / this.rows) - 1) * 4 - (index >= this.capacity / this.rows || !this.playerInventory || this.verticalGap != 0 ? 0 : 12)));
      ...
      for (int index = 0; index < this.capacity; ++index)
      {
        Vector2 location = new Vector2((float) (this.xPositionOnScreen + index % (this.capacity / this.rows) * 64 /*0x40*/ + this.horizontalGap * (index % (this.capacity / this.rows))), (float) (this.yPositionOnScreen + index / (this.capacity / this.rows) * (64 /*0x40*/ + this.verticalGap) + (index / (this.capacity / this.rows) - 1) * 4 - (index >= this.capacity / this.rows || !this.playerInventory || this.verticalGap != 0 ? 0 : 12)));
  ...
  public List<Vector2> GetSlotDrawPositions()
  {
    ...
    for (int index = 0; index < this.capacity; ++index)
      slotDrawPositions.Add(new Vector2((float) (this.xPositionOnScreen + index % (this.capacity / this.rows) * 64 /*0x40*/ + this.horizontalGap * (index % (this.capacity / this.rows))), (float) (this.yPositionOnScreen + index / (this.capacity / this.rows) * (64 /*0x40*/ + this.verticalGap) + (index / (this.capacity / this.rows) - 1) * 4 - (index >= this.capacity / this.rows || !this.playerInventory || this.verticalGap != 0 ? 0 : 12))));
