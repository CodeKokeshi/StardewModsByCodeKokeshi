...
public class MineElevatorMenu : IClickableMenu
{
    ...
    : base(0, 0, 0, 0, true)
  {
    ...
    this.height = Math.Max(64 /*0x40*/ + IClickableMenu.borderWidth * 3, num1 * 44 / (this.width - IClickableMenu.borderWidth) * 44 + 64 /*0x40*/ + IClickableMenu.borderWidth * 3);
  ...
  public override void draw(SpriteBatch b)
  {
    ...
    Game1.drawDialogueBox(this.xPositionOnScreen, this.yPositionOnScreen - 64 /*0x40*/ + 8, this.width + 21, this.height + 64 /*0x40*/, false, true);
    foreach (ClickableComponent elevator in this.elevators)
    {
      b.Draw(Game1.mouseCursors, new Vector2((float) (elevator.bounds.X - 4), (float) (elevator.bounds.Y + 4)), new Rectangle?(new Rectangle((double) elevator.scale > 1.0 ? 267 : 256 /*0x0100*/, 256 /*0x0100*/, 10, 10)), Color.Black * 0.5f, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.865f);
      b.Draw(Game1.mouseCursors, new Vector2((float) elevator.bounds.X, (float) elevator.bounds.Y), new Rectangle?(new Rectangle((double) elevator.scale > 1.0 ? 267 : 256 /*0x0100*/, 256 /*0x0100*/, 10, 10)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, 0.868f);
      Vector2 position = new Vector2((float) (elevator.bounds.X + 16 /*0x10*/ + NumberSprite.numberOfDigits(Convert.ToInt32(elevator.name)) * 6), (float) (elevator.bounds.Y + 24 - NumberSprite.getHeight() / 4));
