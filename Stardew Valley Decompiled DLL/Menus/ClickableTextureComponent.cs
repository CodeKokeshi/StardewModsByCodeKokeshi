...
public class ClickableTextureComponent : ClickableComponent
{
  ...
  public virtual void draw(SpriteBatch b)
  {
    if (!this.visible)
      return;
    this.draw(b, Color.White, (float) (0.86000001430511475 + (double) this.bounds.Y / 20000.0));
  }
    ...
    int yOffset = 0)
  {
    ...
    if (this.drawLabelWithShadow)
      Utility.drawTextWithShadow(b, this.label, Game1.smallFont, new Vector2((float) (this.bounds.X + xOffset + this.bounds.Width), (float) (this.bounds.Y + yOffset) + ((float) (this.bounds.Height / 2) - Game1.smallFont.MeasureString(this.label).Y / 2f)), Game1.textColor);
    else
