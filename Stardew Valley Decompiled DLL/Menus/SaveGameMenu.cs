...
public class SaveGameMenu : IClickableMenu, IDisposable
{
  ...
  public SaveGameMenu()
  {
    this.saveText = new SparklingText(Game1.dialogueFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:SaveGameMenu.cs.11378"), Color.LimeGreen, Color.Black * (1f / 1000f), millisecondsDuration: 1500, amplitude: 32 /*0x20*/);
  ...
  {
    base.draw(b);
    Vector2 vector2 = Utility.makeSafe(new Vector2(64f, (float) (Game1.uiViewport.Height - 64 /*0x40*/)), new Vector2(64f, 64f));
    bool flag = false;
    if (this.completePause >= 0)
    {
      if (Game1.saveOnNewDay)
    ...
    else if (this.margin < 0 || Game1.IsClient)
    {
      ...
      {
        int sub2 = Game1.getOnlineFarmers().Count - (Game1.HasDedicatedHost ? 1 : 0);
        this._stringBuilder.Clear();
        this._stringBuilder.Append(Game1.content.LoadString("Strings\\UI:ReadyCheck", (object) Game1.newDaySync.numReadyForSave(), (object) sub2));
