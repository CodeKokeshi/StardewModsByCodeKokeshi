...
public class LoadGameMenu : IClickableMenu, IDisposable
{
    ...
    : base(Game1.uiViewport.Width / 2 - (1100 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2, 1100 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2)
  {
    ...
    ClickableTextureComponent textureComponent1 = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + this.width + 16 /*0x10*/, this.yPositionOnScreen + 16 /*0x10*/, 44, 48 /*0x30*/), Game1.mouseCursors, new Rectangle(421, 459, 11, 12), 4f);
    ...
    ClickableTextureComponent textureComponent2 = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + this.width + 16 /*0x10*/, this.yPositionOnScreen + this.height - 64 /*0x40*/, 44, 48 /*0x30*/), Game1.mouseCursors, new Rectangle(421, 472, 11, 12), 4f);
    ...
    ClickableTextureComponent textureComponent3 = new ClickableTextureComponent(Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.10992"), new Rectangle((int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).X - 64 /*0x40*/, (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).Y + 128 /*0x80*/, 64 /*0x40*/, 64 /*0x40*/), (string) null, (string) null, Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 46), 1f);
    ...
    ClickableTextureComponent textureComponent4 = new ClickableTextureComponent(Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.10993"), new Rectangle((int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).X + 64 /*0x40*/, (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).Y + 128 /*0x80*/, 64 /*0x40*/, 64 /*0x40*/), (string) null, (string) null, Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 47), 1f);
    ...
    {
      this.slotButtons.Add(new ClickableComponent(new Rectangle(this.xPositionOnScreen + 16 /*0x10*/, this.yPositionOnScreen + 16 /*0x10*/ + index * (this.height / 4), this.width - 32 /*0x20*/, this.height / 4 + 4), index.ToString() ?? "")
      {
        myID = index,
        region = 900,
      ...
      if (this.hasDeleteButtons())
      {
        ...
        ClickableTextureComponent textureComponent5 = new ClickableTextureComponent("", new Rectangle(this.xPositionOnScreen + this.width - 64 /*0x40*/ - 4, this.yPositionOnScreen + 32 /*0x20*/ + 4 + index * (this.height / 4), 48 /*0x30*/, 48 /*0x30*/), "", Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.10994"), Game1.mouseCursors, new Rectangle(322, 498, 12, 12), 3f);
  ...
  {
    int num = this.MenuSlots.Count + 1;
    for (int index = 0; index < files.Count; ++index)
    {
      Farmer file = files[index];
      if (file != null)
        this.MenuSlots.Add((LoadGameMenu.MenuSlot) new LoadGameMenu.SaveFileSlot(this, file, new int?(num + index)));
    }
    this.UpdateButtons();
  }
  ...
  private static List<Farmer> FindSaveGames(string filter)
  {
    ...
    {
      foreach (string path2 in Directory.EnumerateDirectories(savesFolder).ToList<string>())
      {
        string path3 = ((IEnumerable<string>) path2.Split(Path.DirectorySeparatorChar)).Last<string>();
        string pathToFile = Path.Combine(savesFolder, path2, "SaveGameInfo");
        string pathToSave = Path.Combine(savesFolder, path2, path3);
        if (File.Exists(pathToSave) || File.Exists(pathToSave + "_old") || File.Exists(pathToSave + "_STARDEWVALLEYSAVETMP"))
        {
          ...
          try
          {
            Exception loadError1;
            Exception loadError2;
            target = TryReadSaveInfo((string) null, out loadError1) ?? (TryReadSaveInfo("_old", out loadError2) ?? TryReadSaveInfo("_STARDEWVALLEYSAVETMP", out loadError2)) ?? TryReadSaveData((string) null, out loadError2) ?? TryReadSaveData("_old", out loadError2) ?? TryReadSaveData("_STARDEWVALLEYSAVETMP", out loadError2);
            if (target == null)
            {
              Game1.log.Error($"Exception occurred trying to access file '{pathToFile}'", loadError1);
            }
            else
            {
          ...
          {
            Game1.log.Error($"Exception occurred trying to access file '{pathToFile}'", ex);
            target?.unload();
          }
        }
        ...
        {
          return TryReadFile<Farmer>(pathToFile + suffix, out loadError, new Func<Stream, Farmer>(SaveSerializer.Deserialize<Farmer>));
        }

        Farmer TryReadSaveData(string suffix, out Exception loadError)
        {
          return TryReadFile<SaveGame>(pathToSave + suffix, out loadError, new Func<Stream, SaveGame>(SaveSerializer.Deserialize<SaveGame>))?.player;
        }
      }
    }
    saveGames.Sort();
    ...
    {
      for (int index = 0; index < saveGames.Count; ++index)
      {
        Farmer farmer = saveGames[index];
        string name = farmer.Name;
        if ((name != null ? (name.IndexOfIgnoreCase(filter) == -1 ? 1 : 0) : 0) != 0)
        {
          string str = farmer.farmName.Value;
          if ((str != null ? (str.IndexOfIgnoreCase(filter) == -1 ? 1 : 0) : 0) != 0)
            saveGames[index] = (Farmer) null;
        }
      }
    }
    return saveGames;
  ...
  public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
  {
    ...
    this.yPositionOnScreen = (newBounds.Height - (this.height + 32 /*0x20*/)) / 2;
    ...
    this.upArrow.bounds.X = this.xPositionOnScreen + this.width + 16 /*0x10*/;
    this.upArrow.bounds.Y = this.yPositionOnScreen + 16 /*0x10*/;
    this.downArrow.bounds.X = this.xPositionOnScreen + this.width + 16 /*0x10*/;
    this.downArrow.bounds.Y = this.yPositionOnScreen + this.height - 64 /*0x40*/;
    this.scrollBar = new ClickableTextureComponent(new Rectangle(this.upArrow.bounds.X + 12, this.upArrow.bounds.Y + this.upArrow.bounds.Height + 4, 24, 40), Game1.mouseCursors, new Rectangle(435, 463, 6, 10), 4f);
    this.scrollBarRunner = new Rectangle(this.scrollBar.bounds.X, this.upArrow.bounds.Y + this.upArrow.bounds.Height + 4, this.scrollBar.bounds.Width, this.height - 64 /*0x40*/ - this.upArrow.bounds.Height - 28);
    this.okDeleteButton.bounds.X = (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).X - 64 /*0x40*/;
    this.okDeleteButton.bounds.Y = (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).Y + 128 /*0x80*/;
    this.cancelDeleteButton.bounds.X = (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).X + 64 /*0x40*/;
    this.cancelDeleteButton.bounds.Y = (int) Utility.getTopLeftPositionForCenteringOnScreen(64 /*0x40*/, 64 /*0x40*/).Y + 128 /*0x80*/;
    for (int index = 0; index < this.slotButtons.Count; ++index)
    {
      this.slotButtons[index].bounds.X = this.xPositionOnScreen + 16 /*0x10*/;
      this.slotButtons[index].bounds.Y = this.yPositionOnScreen + 16 /*0x10*/ + index * (this.height / 4);
    }
    for (int index = 0; index < this.deleteButtons.Count; ++index)
    {
      this.deleteButtons[index].bounds.X = this.xPositionOnScreen + this.width - 64 /*0x40*/ - 4;
      this.deleteButtons[index].bounds.Y = this.yPositionOnScreen + 32 /*0x20*/ + 4 + index * (this.height / 4);
    }
    if (!Game1.options.snappyMenus || !Game1.options.gamepadControls)
      return;
  ...
  private void deleteFile(int which)
  {
    ...
    for (int index = 0; index < 50 && Directory.Exists(path); ++index)
      Thread.Sleep(100);
  }
  ...
  protected virtual void saveFileScanComplete() => Game1.game1.ResetGameStateOnTitleScreen();
    ...
    else
    {
      if (this.selectedForDelete != -1 && !this.deleteConfirmationScreen && !this.deleting && this.MenuSlots[this.selectedForDelete] is LoadGameMenu.SaveFileSlot menuSlot)
      {
        ...
        for (int index = 0; index < 4; ++index)
        {
          this.slotButtons.Add(new ClickableComponent(new Rectangle(this.xPositionOnScreen + 16 /*0x10*/, this.yPositionOnScreen + 16 /*0x10*/ + index * (this.height / 4), this.width - 32 /*0x20*/, this.height / 4 + 4), index.ToString() ?? ""));
          if (this.hasDeleteButtons())
            this.deleteButtons.Add(new ClickableTextureComponent("", new Rectangle(this.xPositionOnScreen + this.width - 64 /*0x40*/ - 4, this.yPositionOnScreen + 32 /*0x20*/ + 4 + index * (this.height / 4), 48 /*0x30*/, 48 /*0x30*/), "", "Delete File", Game1.mouseCursors, new Rectangle(322, 498, 12, 12), 3f));
        }
  ...
  protected virtual void drawStatusText(SpriteBatch b)
  {
    ...
    SpriteText.drawStringHorizontallyCenteredAt(b1, s, x, y, color: color);
  }
  ...
  {
    this.drawBefore(b);
    IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 373, 18, 18), this.xPositionOnScreen, this.yPositionOnScreen, this.width, this.height + 32 /*0x20*/, Color.White, 4f);
    ...
    if (this.deleteConfirmationScreen && this.MenuSlots[this.selectedForDelete] is LoadGameMenu.SaveFileSlot menuSlot)
    {
      b.Draw(Game1.staminaRect, new Rectangle(0, 0, Game1.uiViewport.Width, Game1.uiViewport.Height), Color.Black * 0.75f);
      string s = Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11023", (object) menuSlot.Farmer.Name);
      ...
      SpriteText.drawString(b, s, num - SpriteText.getWidthOfString(s) / 2, (int) Utility.getTopLeftPositionForCenteringOnScreen(192 /*0xC0*/, 64 /*0x40*/).Y, 9999, height: 9999, layerDepth: 1f, color: new Color?(SpriteText.color_White));
    ...
    if (Game1.activeClickableMenu == this && (!Game1.options.SnappyMenus || this.currentlySnappedComponent != null) && !this.IsDoingTask())
      this.drawMouse(b, cursor: this.loading ? 1 : -1);
    this.drawn = true;
  }
  ...
  public class SaveFileSlot : LoadGameMenu.MenuSlot
  {
    ...
    protected virtual void drawSlotSaveNumber(SpriteBatch b, int i)
    {
      ...
      ClickableComponent slotButton = this.menu.slotButtons[i];
      string s = ((menuSlot is LoadGameMenu.SaveFileSlot saveFileSlot ? saveFileSlot.SlotNumber : new int?()) ?? this.menu.currentItemIndex + i + 1).ToString() + ".";
      SpriteText.drawString(b, s, slotButton.bounds.X + 28 + 32 /*0x20*/ - SpriteText.getWidthOfString(s) / 2, slotButton.bounds.Y + 36);
    }
    ...
    {
      SpriteText.drawString(b, this.slotName(), this.menu.slotButtons[i].bounds.X + 128 /*0x80*/ + 36, this.menu.slotButtons[i].bounds.Y + 36, alpha: this.getSlotAlpha());
    }
    ...
    protected virtual void drawSlotShadow(SpriteBatch b, int i)
    {
      ...
      Vector2 position = new Vector2((float) ((double) this.menu.slotButtons[i].bounds.X + (double) vector2.X + 32.0), (float) (this.menu.slotButtons[i].bounds.Y + 128 /*0x80*/ + 16 /*0x10*/));
    ...
    protected virtual void drawSlotFarmer(SpriteBatch b, int i)
    {
      ...
      this.Farmer.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(0, 0, false, false), 0, new Rectangle(0, 0, 16 /*0x10*/, 32 /*0x20*/), new Vector2((float) this.menu.slotButtons[i].bounds.X + vector2.X, (float) this.menu.slotButtons[i].bounds.Y + vector2.Y), Vector2.Zero, 0.8f, 2, Color.White, 0.0f, 1f, this.Farmer);
    ...
    protected virtual void drawSlotDate(SpriteBatch b, int i)
    {
      ...
      Utility.drawTextWithShadow(b, text, Game1.dialogueFont, new Vector2((float) (this.menu.slotButtons[i].bounds.X + 128 /*0x80*/ + 32 /*0x20*/), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 40)), Game1.textColor * this.getSlotAlpha());
    }

    protected virtual string slotSubName()
    {
      return Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11019", (object) this.Farmer.farmName);
    }

    protected virtual void drawSlotSubName(SpriteBatch b, int i)
    {
      string text = this.slotSubName();
      Utility.drawTextWithShadow(b, text, Game1.dialogueFont, new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 128 /*0x80*/) - Game1.dialogueFont.MeasureString(text).X, (float) (this.menu.slotButtons[i].bounds.Y + 44)), Game1.textColor * this.getSlotAlpha());
    }
    ...
    protected virtual void drawSlotMoney(SpriteBatch b, int i)
    {
      ...
      Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 192 /*0xC0*/ - 100 - x), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 44)), new Rectangle(193, 373, 9, 9), Color.White, 0.0f, Vector2.Zero, 4f, layerDepth: 1f);
      Vector2 position = new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 192 /*0xC0*/ - 60 - x), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 44));
    ...
    {
      Vector2 position = new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 192 /*0xC0*/ - 44), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 36));
      ...
      position = new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 192 /*0xC0*/ - 4), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 44));
      if (LocalizedContentManager.CurrentLanguageCode != LocalizedContentManager.LanguageCode.en)
        position.Y += 5f;
    ...
    {
      SpriteText.drawString(b, this.slotName(), this.menu.slotButtons[i].bounds.X + 128 /*0x80*/, this.menu.slotButtons[i].bounds.Y + 36);
      string text1 = this.slotSubName();
      Utility.drawTextWithShadow(b, text1, Game1.dialogueFont, new Vector2((float) (this.menu.slotButtons[i].bounds.X + this.menu.width - 128 /*0x80*/) - Game1.dialogueFont.MeasureString(text1).X, (float) (this.menu.slotButtons[i].bounds.Y + 44)), Game1.textColor);
      ...
      if (Game1.currentGameTime.TotalGameTime.TotalSeconds < this.redTimer && (int) ((this.redTimer - Game1.currentGameTime.TotalGameTime.TotalSeconds) / 0.25) % 2 == 1)
        color = Color.Red;
      Utility.drawTextWithShadow(b, text2, Game1.dialogueFont, new Vector2((float) (this.menu.slotButtons[i].bounds.X + 128 /*0x80*/), (float) (this.menu.slotButtons[i].bounds.Y + 64 /*0x40*/ + 40)), color);
    }
