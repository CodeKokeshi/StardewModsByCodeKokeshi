...
// Type: StardewValley.Extensions.RandomExtensions
...
public static class RandomExtensions
{
  ...
  {
    return random.NextDouble() >= 0.5 ? optionB : optionA;
  }
  ...
  {
    return options == null || options.Length == 0 ? default (T) : options[random.Next(options.Length)];
  }
  ...
  {
    return options == null || options.Count <= 0 ? default (T) : options[random.Next(options.Count)];
  }
  ...
  {
    return (double) chance >= 1.0 || random.NextDouble() < (double) chance;
  }
}
