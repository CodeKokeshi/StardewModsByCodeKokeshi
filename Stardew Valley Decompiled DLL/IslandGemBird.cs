...
public class IslandGemBird : INetObject<NetFields>
{
  ...
  [XmlIgnore]
  public NetFields NetFields { get; } = new NetFields(nameof (IslandGemBird));
  ...
  public static IslandGemBird.GemBirdType GetBirdTypeForLocation(string location)
  {
    ...
    Utility.Shuffle<IslandGemBird.GemBirdType>(random, list);
  ...
  public void Draw(SpriteBatch b)
  {
    ...
    b.Draw(this.texture, Game1.GlobalToLocal(Game1.viewport, this.position.Value + new Vector2(0.0f, -this.height.Value)), new Rectangle?(new Rectangle(num * 32 /*0x20*/, 0, 32 /*0x20*/, 32 /*0x20*/)), Color.White * this.alpha.Value, 0.0f, new Vector2(16f, 32f), 4f, SpriteEffects.None, (float) (((double) this.position.Value.Y - 1.0) / 10000.0));
    b.Draw(this.texture, Game1.GlobalToLocal(Game1.viewport, this.position.Value + new Vector2(0.0f, -this.height.Value)), new Rectangle?(new Rectangle(num * 32 /*0x20*/, 32 /*0x20*/, 32 /*0x20*/, 32 /*0x20*/)), this.color.Value * this.alpha.Value, 0.0f, new Vector2(16f, 32f), 4f, SpriteEffects.None, this.position.Value.Y / 10000f);
    b.Draw(Game1.shadowTexture, Game1.GlobalToLocal(Game1.viewport, this.position.Value), new Rectangle?(Game1.shadowTexture.Bounds), Color.White * this.alpha.Value, 0.0f, new Vector2((float) Game1.shadowTexture.Bounds.Center.X, (float) Game1.shadowTexture.Bounds.Center.Y), 3f, SpriteEffects.None, (float) (((double) this.position.Y - 2.0) / 10000.0));
  }

  public void InitNetFields()
  {
    this.NetFields.SetOwner((INetObject<NetFields>) this).AddField((INetSerializable) this.position, "position").AddField((INetSerializable) this.flying, "flying").AddField((INetSerializable) this.height, "height").AddField((INetSerializable) this.color, "color").AddField((INetSerializable) this.alpha, "alpha").AddField((INetSerializable) this.itemIndex, "itemIndex");
  ...
  public bool Update(GameTime time, GameLocation location)
  {
    ...
    if ((double) this.frameTimer >= (double) num)
    {
      ...
      if (this.currentFrameIndex >= this.currentAnimation.Length)
      {
        ...
        if (this.currentAnimation == this.flyAnimation && location == Game1.currentLocation && Utility.isOnScreen(this.position.Value + new Vector2(0.0f, -this.height.Value), 64 /*0x40*/))
    ...
    if (this.flying.Value)
    {
      ...
      if (Game1.IsMasterGame)
      {
        ...
        this.position.X -= 3f;
        if ((double) this.alpha.Value > 0.0 && (double) this.height.Value >= 300.0)
        {
