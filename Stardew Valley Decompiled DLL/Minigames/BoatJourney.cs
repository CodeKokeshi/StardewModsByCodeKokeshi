...
public class BoatJourney : IMinigame
{
  ...
  public BoatJourney()
  {
    ...
    Game1.changeMusicTrack("sweet", music_context: MusicContext.MiniGame);
    ...
    Rectangle r = new Rectangle(0, 112 /*0x70*/, 640, 528);
    ...
    for (int index = 0; index < 8; ++index)
    {
      ...
      if (Game1.random.NextDouble() < 0.44999998807907104)
        ...
        source_rect = new Rectangle(640, 256 /*0x0100*/, 150, 80 /*0x50*/);
      this.entities.Add(new BoatJourney.Entity(this, BoatJourney.GetAssetName(), source_rect, new Vector2((float) (source_rect.Width / 2), (float) source_rect.Height), positionInThisRectangle)
      {
    ...
    for (int index = 0; index < 2; ++index)
    {
      if (Game1.random.NextDouble() < 0.30000001192092896)
        this.SpawnBoat(new Rectangle(640, 416, 32 /*0x20*/, 32 /*0x20*/), new Vector2(-1f, 0.0f), other_boat_positions);
    }
    if (Game1.random.NextDouble() < 0.20000000298023224)
      this.SpawnBoat(new Rectangle(704, 416, 32 /*0x20*/, 32 /*0x20*/), new Vector2(-1f, 0.0f), other_boat_positions);
    for (int index = 0; index < 2; ++index)
    {
      if (Game1.random.NextDouble() < 0.30000001192092896)
        this.SpawnBoat(new Rectangle(640, 448, 32 /*0x20*/, 32 /*0x20*/), new Vector2(1f, 0.0f), other_boat_positions);
    }
    ...
    this._boat = new BoatJourney.Boat(this, BoatJourney.GetAssetName(), new Rectangle(640, 352, 32 /*0x20*/, 32 /*0x20*/), new Vector2(16f, 16f), new Vector2(293f, 53f));
    ...
    this.entities.Add(new BoatJourney.Entity(this, BoatJourney.GetAssetName(), new Rectangle(643, 538, 29, 17), Vector2.Zero, new Vector2(16f, 829f))
    {
      numFrames = 2,
      frameInterval = 0.75f
    });
  ...
  private static string GetAssetName() => $"Minigames\\{Game1.currentSeason}_boatJourneyMap";
    ...
    List<Vector2> other_boat_positions)
  {
    ...
    do
    {
      ...
      do
      {
        vector2 = Game1.random.ChooseFrom<Vector2>((IList<Vector2>) this._interpolatedPoints);
      }
      while (!new Rectangle(0, 112 /*0x70*/, 640, 528).Contains((int) vector2.X, (int) vector2.Y));
      position = vector2 + direction * Utility.RandomFloat(8f, 64f);
      flag = false;
    ...
    while (flag);
    BoatJourney.Boat boat = new BoatJourney.Boat(this, BoatJourney.GetAssetName(), boat_sprite_rect, new Vector2(16f, 14f), position);
  ...
  public void CreateFlockOfSeagulls(int x, int y, int depth)
  {
    ...
    BoatJourney.Entity entity1 = new BoatJourney.Entity(this, BoatJourney.GetAssetName(), new Rectangle(646, 560, 5, 14), new Vector2(2f, 14f), new Vector2((float) x, (float) y));
    entity1.numFrames = 8;
    ...
    for (int index = 1; index < depth; ++index)
    {
      ...
      BoatJourney.Entity entity2 = new BoatJourney.Entity(this, BoatJourney.GetAssetName(), new Rectangle(646, 560, 5, 14), new Vector2(2f, 14f), position1);
      entity2.numFrames = 8;
      ...
      BoatJourney.Entity entity3 = new BoatJourney.Entity(this, BoatJourney.GetAssetName(), new Rectangle(646, 560, 5, 14), new Vector2(2f, 14f), position2);
      entity3.numFrames = 8;
  ...
  public bool tick(GameTime time)
  {
    ...
    double age = (double) this._age;
    TimeSpan elapsedGameTime = time.ElapsedGameTime;
    double totalSeconds1 = elapsedGameTime.TotalSeconds;
    this._age = (float) (age + totalSeconds1);
    this.entities.RemoveAll((Predicate<BoatJourney.Entity>) (entity => entity.Update(time)));
    ...
    if ((seagulls != null ? (__nonvirtual (seagulls.Count) > 0 ? 1 : 0) : 0) != 0 && (double) this._boat.position.Y > (double) this._seagulls[0].position.Y)
    {
    ...
    if (this._interpolatedPoints.Count > 1)
    {
      ...
      {
        double departureDelay = (double) this.departureDelay;
        elapsedGameTime = time.ElapsedGameTime;
        double totalSeconds2 = elapsedGameTime.TotalSeconds;
        this.departureDelay = (float) (departureDelay - totalSeconds2);
      }
      else
      {
        ...
        this._nextSlosh = (float) (nextSlosh - totalSeconds3);
      ...
      else
      {
        ...
        if ((double) Math.Abs(vector2.X) > (double) Math.Abs(vector2.Y))
        {
          if ((double) vector2.X < 0.0)
            this._boat.SetSourceRect(new Rectangle(704, 384, 32 /*0x20*/, 32 /*0x20*/));
          else
            this._boat.SetSourceRect(new Rectangle(704, 352, 32 /*0x20*/, 32 /*0x20*/));
        }
        else if ((double) vector2.Y > 0.0)
          this._boat.SetSourceRect(new Rectangle(640, 384, 32 /*0x20*/, 32 /*0x20*/));
        else
          this._boat.SetSourceRect(new Rectangle(640, 352, 32 /*0x20*/, 32 /*0x20*/));
    ...
    if ((double) this.nextSmoke <= 0.0)
    {
      ...
      BoatJourney.Entity entity = new BoatJourney.Entity(this, BoatJourney.GetAssetName(), new Rectangle(640, 480, 16 /*0x10*/, 16 /*0x10*/), new Vector2(8f, 8f), new Vector2(350f, 665f));
      entity.numFrames = 7;
    ...
    {
      double nextSmoke = (double) this.nextSmoke;
      elapsedGameTime = time.ElapsedGameTime;
      double totalSeconds4 = elapsedGameTime.TotalSeconds;
      this.nextSmoke = (float) (nextSmoke - totalSeconds4);
    }
    return false;
  }
  ...
  public Color getWaterColorForSeason()
  {
    switch (Game1.season)
    {
      ...
      case Season.Fall:
        return new Color(56, 70, 128 /*0x80*/);
      case Season.Winter:
  ...
  public void draw(SpriteBatch b)
  {
    ...
    b.Draw(Game1.staminaRect, new Rectangle(0, 0, Game1.viewport.Width, Game1.viewport.Height), new Rectangle?(), this.getWaterColorForSeason(), 0.0f, Vector2.Zero, SpriteEffects.None, 0.0f);
    b.Draw(Game1.staminaRect, this.TransformDraw(new Rectangle(-Game1.viewport.Width, 400, Game1.viewport.Width * 3, Game1.viewport.Height)), new Rectangle?(), new Color(49, 79, 155), 0.0f, Vector2.Zero, SpriteEffects.None, 5E-06f);
    ...
    b.Draw(this.texture, this.TransformDraw(new Rectangle(-640, 331, 640, 294)), new Rectangle?(new Rectangle(0, 337, 640, 294)), Color.White, 0.0f, Vector2.Zero, SpriteEffects.None, 1E-05f);
    b.Draw(this.texture, this.TransformDraw(new Rectangle(640, 343, 640, 294)), new Rectangle?(new Rectangle(0, 337, 640, 294)), Color.White, 0.0f, Vector2.Zero, SpriteEffects.None, 1E-05f);
  ...
  public void unload() => Game1.stopMusicTrack(MusicContext.MiniGame);
  ...
  public class WaterSparkle : BoatJourney.Entity
  {
    ...
    public WaterSparkle(BoatJourney context)
      : base(context, BoatJourney.GetAssetName(), new Rectangle(647, 524, 1, 1), new Vector2(0.0f, 0.0f), new Vector2(0.0f, 0.0f))
    {
    ...
    {
      return (double) this.layerDepth >= 0.0 ? this.layerDepth : 0.0001f;
    }
  }
  ...
  public class Wave : BoatJourney.Entity
  {
    ...
    public Wave(BoatJourney context, Vector2 position = default (Vector2))
      : base(context, BoatJourney.GetAssetName(), new Rectangle(640, 506, 32 /*0x20*/, 12), new Vector2(16f, 6f), position)
    {
      this.numFrames = 2;
    ...
    {
      this.position = this._startPosition + new Vector2(1f, 0.0f) * (float) Math.Sin((double) this._startPosition.X * 0.33300000429153442 + (double) this._startPosition.Y * 0.10000000149011612 + (double) this._age) * 3f;
      return base.Update(time);
    }

    public override float GetLayerDepth()
    {
      return (double) this.layerDepth >= 0.0 ? this.layerDepth : 0.0003f;
    }
  }
  ...
  public class Boat(
    ...
    Vector2 position = default (Vector2)) : BoatJourney.Entity(context, texture_path, source_rect, origin, position)
  {
    ...
    public override bool Update(GameTime time)
    {
      ...
      if (this.smokeStack.HasValue)
      {
        if ((double) this.nextSmokeStackSmoke <= 0.0)
        {
          ...
          if (flag)
          {
            BoatJourney.Entity entity = new BoatJourney.Entity(this._context, BoatJourney.GetAssetName(), new Rectangle(689, 337, 2, 2), new Vector2(1f, 1f), this.position + this.smokeStack.Value);
            entity.numFrames = 3;
      ...
      if ((double) this.nextRipple <= 0.0)
      {
        ...
        if (flag)
          this._context.entities.Add(new BoatJourney.Entity(this._context, BoatJourney.GetAssetName(), new Rectangle(640, 336, 9, 16 /*0x10*/), new Vector2(4f, 0.0f), this.position + new Vector2(0.0f, 0.0f))
          {
            numFrames = 5,
            layerDepth = 2E-05f,
  ...
  public class Entity
  {
    ...
    {
      return (double) this.layerDepth >= 0.0 ? this.layerDepth : this.position.Y / 100000f;
    }

    public virtual void Draw(SpriteBatch b)
    {
      b.Draw(this._texture, this._context.TransformDraw(this.position), new Rectangle?(this.GetSourceRect()), Color.White, 0.0f, this.origin, this._context._zoomLevel, this.flipX ? SpriteEffects.FlipHorizontally : SpriteEffects.None, this.GetLayerDepth());
    }
  }
}
