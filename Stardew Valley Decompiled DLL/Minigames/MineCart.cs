...
public class MineCart : IMinigame
{
  ...
  public const int followDistance = 96 /*0x60*/;
  public float pixelScale = 4f;
  public const int tilesBeyondViewportToSimulate = 4;
  public const int bgLoopWidth = 96 /*0x60*/;
  ...
  public const int checkpointScanDistance = 16 /*0x10*/;
  ...
  private Rectangle midBGSource = new Rectangle(64 /*0x40*/, 0, 96 /*0x60*/, 162);
  private Rectangle backBGSource = new Rectangle(64 /*0x40*/, 162, 96 /*0x60*/, 111);
  private Rectangle lakeBGSource = new Rectangle(0, 80 /*0x50*/, 16 /*0x10*/, 97);
  ...
  public MineCart(int whichTheme, int mode)
  {
    ...
    Game1.playSound(nameof (minecartLoop), out this.minecartLoop);
  ...
  public void initLevelTransitions()
  {
    this.LEVEL_TRANSITIONS = new MineCart.LevelTransition[15]
    {
      ...
      new MineCart.LevelTransition(1, 5, 6, 11, "rrurruuu", new Func<bool>(((RandomExtensions) Game1.random).NextBool)),
      new MineCart.LevelTransition(1, 2, 6, 11, "rrurrrrddr"),
      new MineCart.LevelTransition(8, 5, 8, 8, "ddrruuu", new Func<bool>(((RandomExtensions) Game1.random).NextBool)),
      ...
      new MineCart.LevelTransition(3, 9, 16 /*0x10*/, 8, "rruuluu", new Func<bool>(((RandomExtensions) Game1.random).NextBool)),
      new MineCart.LevelTransition(3, 4, 16 /*0x10*/, 8, "rrddrddr"),
  ...
  public void ShowTitle()
  {
    ...
    Game1.changeMusicTrack("junimoKart", music_context: MusicContext.MiniGame);
    ...
    this.titleScreenJunimo = this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(259, 492, 14, 20), new Vector2((float) (this.screenWidth / 2 - 128 /*0x80*/ + 137), (float) (this.screenHeight / 2 - 35 + 46)), 100f, 0.0f, gravity_multiplier: 0.0f, life_time: 99999f, animation_interval: 1f, draw_depth: 0.24f));
    if (this.gameMode == 3)
      this.setUpTheme(-1);
    else
      this.setUpTheme(0);
  ...
  {
    List<Type> options;
    if (track == null || !this._validObstacles.TryGetValue(obstacle_type, out options))
      return (MineCart.Obstacle) null;
    MineCart.Obstacle obstacle = this.AddEntity<MineCart.Obstacle>(Activator.CreateInstance(Game1.random.ChooseFrom<Type>((IList<Type>) options)) as MineCart.Obstacle);
  ...
  {
    List<MineCart.Track> trackList;
    return !this._tracks.TryGetValue((int) ((double) x / (double) this.tileSize), out trackList) ? (MineCart.Track) null : trackList[0];
  }
  ...
  {
    return this._tracks.GetValueOrDefault<int, List<MineCart.Track>>((int) ((double) x / (double) this.tileSize));
  }

  protected bool _IsGeneratingOnUpperHalf()
  {
    return this.generatorPosition.Y <= (this.topTile + this.bottomTile) / 2;
  }
  ...
  {
    while ((double) (this.generatorPosition.X * this.tileSize) <= (double) this.screenLeftBound + (double) this.screenWidth + (double) (16 /*0x10*/ * this.tileSize))
    {
  ...
  public MineCart.Track AddTrack(MineCart.Track track_object)
  {
    ...
    List<MineCart.Track> source;
    if (!this._tracks.TryGetValue(key, out source))
      this._tracks[key] = source = new List<MineCart.Track>();
    source.Add(track_object);
    source.OrderBy<MineCart.Track, float>((Func<MineCart.Track, float>) (o => o.position.Y));
    return track;
  }
  ...
  {
    this.mapTimer += time;
    MineCart.MapJunimo mapJunimo1 = (MineCart.MapJunimo) null;
    foreach (MineCart.Entity entity in this._entities)
    {
      if (entity is MineCart.MapJunimo mapJunimo2)
      {
        mapJunimo1 = mapJunimo2;
        break;
      }
    }
    if ((double) this.mapTimer >= 2.0 && mapJunimo1.moveState == MineCart.MapJunimo.MoveState.Idle)
      mapJunimo1.StartMoving();
    switch (mapJunimo1.moveState)
    {
      case MineCart.MapJunimo.MoveState.Moving:
        this.mapTimer = 0.0f;
        break;
      case MineCart.MapJunimo.MoveState.Finished:
        if ((double) this.mapTimer >= 1.5)
        {
          this.fadeDelta = 1f;
          break;
        }
  ...
  public void UpdateCutsceneTick()
  {
    ...
    switch (this.currentTheme)
    {
      ...
      case 6:
        ...
        break;
      case 7:
        this.UpdateCutsceneForEnding(ref fadeOutTimer);
        break;
      case 9:
        this.UpdateCutsceneForMushroomArea();
        break;
    }
    if (this.cutsceneTick == fadeOutTimer)
    {
      this.screenDarkness = 0.0f;
      this.fadeDelta = 2f;
    }
    if (this.cutsceneTick == fadeOutTimer + 100)
    {
      this.EndCutscene();
    }
    else
  ...
  public void UpdateCutsceneForLavaArea()
  {
    switch (this.cutsceneTick)
    {
      case 0:
        ...
        this.AddTrack(16 /*0x10*/, 9, MineCart.Track.TrackType.DownSlope);
        this.AddTrack(17, 8, MineCart.Track.TrackType.UpSlope);
    ...
    switch (this.cutsceneTick)
    {
      ...
      case 250:
        ...
        using (List<MineCart.Entity>.Enumerator enumerator = this._entities.GetEnumerator())
        {
          ...
          {
            if (enumerator.Current is MineCart.Whale current)
            {
              Game1.playSound("croak");
              current.SetState(MineCart.Whale.CurrentState.OpenMouth);
              break;
            }
          }
          break;
    ...
    switch (this.cutsceneTick)
    {
      ...
      default:
        ...
        using (List<MineCart.Entity>.Enumerator enumerator = this._entities.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            MineCart.Entity current = enumerator.Current;
            if (current is MineCart.WillOWisp)
  ...
  public void UpdateCutsceneForMushroomArea()
  {
    switch (this.cutsceneTick)
    {
      case 0:
        ...
        track_object2.position.X = (float) (16 /*0x10*/ * this.tileSize);
  ...
  public void UpdateCutsceneForEnding(ref int fadeOutTimer)
  {
    ...
    switch (this.cutsceneTick)
    {
      case 0:
        ...
        {
          this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 182, 48 /*0x30*/, 45), new Vector2((float) (20 * this.tileSize) + 12f, (float) (10 * this.tileSize) - 21.5f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 1000f, animation_interval: 0.0f, draw_depth: 0.23f, holdLastFrame: true));
          break;
        }
        this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 112 /*0x70*/, 25, 32 /*0x20*/), new Vector2((float) (20 * this.tileSize) + 12f, (float) (10 * this.tileSize) - 16f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 1000f, animation_interval: 0.0f, draw_depth: 0.23f, holdLastFrame: true));
        break;
      case 200:
  ...
  public void UpdateInput()
  {
    ...
    else
    {
      if (this.gamePaused)
        return;
      int num;
      if (Game1.input.GetMouseState().LeftButton != ButtonState.Pressed && !Game1.isOneOfTheseKeysDown(Game1.input.GetKeyboardState(), Game1.options.useToolButton) && !Game1.isOneOfTheseKeysDown(Game1.input.GetKeyboardState(), Game1.options.actionButton))
      {
        ...
        if (!keyboardState.IsKeyDown(Keys.Space))
        {
          ...
          if (!keyboardState.IsKeyDown(Keys.LeftShift))
          {
            ...
            if (!gamePadState.IsButtonDown(Buttons.A))
            {
              ...
              num = gamePadState.IsButtonDown(Buttons.B) ? 1 : 0;
              goto label_10;
            }
          }
        }
...
label_10:
  ...
  public virtual bool CanPause()
  {
    switch (this.gameState)
    {
      case MineCart.GameStates.Ingame:
  ...
  public bool tick(GameTime time)
  {
    ...
    switch (this.gameState)
    {
      case MineCart.GameStates.Title:
        ...
        {
          Stopwatch musicSw = this.musicSW;
          if ((musicSw != null ? (musicSw.ElapsedMilliseconds >= 48000L ? 1 : 0) : 0) != 0)
          {
        ...
        {
          Stopwatch musicSw = this.musicSW;
          if ((musicSw != null ? (musicSw.ElapsedMilliseconds >= 80000L ? 1 : 0) : 0) != 0)
          {
    ...
    this._entities.RemoveAll((Predicate<MineCart.Entity>) (entity => entity != null && entity.ShouldReap()));
    ...
    if (this.gameState == MineCart.GameStates.Ingame)
    {
      ...
      {
        for (int index = 0; index < this.checkpointPositions.Count && (double) this.player.position.X >= (double) (this.checkpointPositions[index] * this.tileSize); index = index - 1 + 1)
        {
          foreach (MineCart.Entity entity in this._entities)
          {
            if (entity is MineCart.CheckpointIndicator checkpointIndicator && (int) ((double) checkpointIndicator.position.X / (double) this.tileSize) == this.checkpointPositions[index])
            {
      ...
      if ((double) (this.generatorPosition.X * this.tileSize) <= (double) this.screenLeftBound + (double) this.screenWidth + (double) (16 /*0x10*/ * this.tileSize))
      ...
      if (this._goalIndicator != null && this.distanceToTravel != -1 && (double) this.player.position.X >= (double) this._goalIndicator.position.X && this.distanceToTravel != -1 && (double) this.player.position.Y <= (double) this._goalIndicator.position.Y * (double) this.tileSize + 4.0 && !this.reachedFinish && (double) this.fadeDelta < 0.0)
      {
        ...
        foreach (MineCart.Entity entity in this._entities)
        {
          switch (entity)
          {
            case MineCart.GoalIndicator goalIndicator:
    ...
    float num6 = (this.screenLeftBound - screenLeftBound) / (float) this.tileSize;
    ...
    {
      this.sparkShower[index].dy += (float) (0.10499999672174454 * ((double) time1 / 0.016666600480675697));
  ...
  {
    int num1;
    for (num1 = 0; num1 < 16 /*0x10*/ && this.GetTrackForXPosition((float) (x_pos * this.tileSize)) == null; ++num1)
      --x_pos;
    for (; num1 < 16 /*0x10*/; ++num1)
    {
  ...
  public void Die()
  {
    if (this.respawnCounter > 0 || (double) this.deathTimer > 0.0 || this.reachedFinish || !this.player.enabled)
      return;
    this.player.OnDie();
    this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(16 /*0x10*/, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/), this.player.position, (float) Game1.random.Next(-80, 81), (float) Game1.random.Next(-100, -49), life_time: 1f));
    this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(32 /*0x20*/, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/), this.player.position + new Vector2(0.0f, -this.player.characterExtraHeight), (float) Game1.random.Next(-80, 81), (float) Game1.random.Next(-150, -99), 0.1f, life_time: 1f, scale: 0.6666667f)).SetColor(Color.Lime);
  ...
  {
    float num = this.checkpointPosition - 96f - (float) (4 * this.tileSize);
    foreach (int key in new List<int>((IEnumerable<int>) this._tracks.Keys))
    {
      if ((double) key < (double) num / (double) this.tileSize)
      {
  ...
  public void receiveKeyPress(Keys k)
  {
    ...
    {
      if ((!this.CanPause() || Game1.options.gamepadControls || !k.Equals((object) Keys.P) && !k.Equals((object) Keys.Enter)) && (!Game1.options.gamepadControls || !Game1.input.GetGamePadState().IsButtonDown(Buttons.Start)))
        return;
  ...
  public void ShowMap()
  {
    ...
    else
    {
      ...
      this.player.enabled = false;
      Game1.changeMusicTrack("none", music_context: MusicContext.MiniGame);
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(261f, 106f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.15f, draw_depth: 0.2f)
      {
        ySinWaveMagnitude = (float) Game1.random.Next(1, 6)
      });
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(276f, 117f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.17f, draw_depth: 0.2f)
      {
        ySinWaveMagnitude = (float) Game1.random.Next(1, 6)
      });
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(234f, 136f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.19f, draw_depth: 0.2f)
      {
        ySinWaveMagnitude = (float) Game1.random.Next(1, 6)
      });
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(264f, 131f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.19f, draw_depth: 0.2f)
      {
        ySinWaveMagnitude = (float) Game1.random.Next(1, 6)
      });
      if (Game1.random.NextDouble() < 0.4)
        this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(256 /*0x0100*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(247f, 119f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.19f, draw_depth: 0.2f)
        {
          ySinWaveMagnitude = (float) Game1.random.Next(1, 6)
        });
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(96 /*0x60*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(327f, 186f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.17f, draw_depth: 0.55f));
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(96 /*0x60*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(362f, 190f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.19f, draw_depth: 0.55f));
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(96 /*0x60*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(299f, 197f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.21f, draw_depth: 0.55f));
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(96 /*0x60*/, 864, 16 /*0x10*/, 16 /*0x10*/), new Vector2(375f, 212f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, animation_interval: 0.16f, draw_depth: 0.55f));
      ...
      this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(0, 736, 48 /*0x30*/, 50), new Vector2(280f, 66f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 2, animation_interval: 0.765f, draw_depth: 0.55f));
      ...
      {
        case 0:
          this.AddEntity<MineCart.CosmeticFallingBoulder>(new MineCart.CosmeticFallingBoulder(72f, new Color(130, 96 /*0x60*/, 79), delayBeforeAppear: 0.45f)).position = new Vector2((float) (40 + Game1.random.Next(40)), -16f);
          if (Game1.random.NextBool())
            this.AddEntity<MineCart.CosmeticFallingBoulder>(new MineCart.CosmeticFallingBoulder(72f, new Color(130, 96 /*0x60*/, 79), 80f, 0.5f)).position = new Vector2((float) (80 /*0x50*/ + Game1.random.Next(40)), -16f);
          if (Game1.random.NextBool())
          {
            this.AddEntity<MineCart.CosmeticFallingBoulder>(new MineCart.CosmeticFallingBoulder(72f, new Color(130, 96 /*0x60*/, 79), 88f, 0.55f)).position = new Vector2((float) (120 + Game1.random.Next(40)), -16f);
            break;
          }
          break;
        case 1:
          ...
          for (int index = 0; index < 4; ++index)
            this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(421, 607, 5, 5), new Vector2(119f, 162f), (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-50, -39), 0.25f, life_time: 0.75f, animation_interval: 1f, timeBeforeDisplay: 0.8f));
          for (int index = 0; index < 4; ++index)
            this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(421, 607, 5, 5), new Vector2(49f, 166f), (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-50, -39), 0.25f, life_time: 0.75f, animation_interval: 1f, timeBeforeDisplay: 1.2f));
          break;
        case 2:
          this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(416, 368, 24, 16 /*0x10*/), new Vector2(217f, 177f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 99f, num_animation_frames: 4, draw_depth: 0.54f, holdLastFrame: true, timeBeforeDisplay: 0.8f));
          ...
          break;
        case 3:
          this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(455, 512 /*0x0200*/, 58, 64 /*0x40*/), new Vector2(250f, 136f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 0.8f, draw_depth: 0.21f)).SetDestroySound("barrelBreak");
          for (int index = 0; index < 32 /*0x20*/; ++index)
            this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(51, 53, 9, 9), new Vector2(250f, 136f) + new Vector2((float) Game1.random.Next(-20, 31 /*0x1F*/), (float) Game1.random.Next(-20, 21)), (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-70, -39), 0.25f, life_time: 0.75f, animation_interval: 1f, timeBeforeDisplay: (float) (0.800000011920929 + 0.0099999997764825821 * (double) index)));
          break;
        case 4:
          ...
          break;
        case 5:
          this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(416, 384, 16 /*0x10*/, 16 /*0x10*/), new Vector2(213f, 34f), 0.0f, 0.0f, gravity_multiplier: 0.0f, life_time: 5f, num_animation_frames: 6, draw_depth: 0.55f)).SetDestroySound("slimedead");
          for (int index = 0; index < 8; ++index)
            this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(427, 607, 6, 6), new Vector2((float) (205 + Game1.random.Next(3, 14)), (float) (26 + Game1.random.Next(6, 14))), (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-60, -39), 0.25f, life_time: 0.75f, animation_interval: 1f, timeBeforeDisplay: (float) (5.0 + (double) index * 0.004999999888241291)));
          break;
        case 6:
          for (int index = 0; index < 52; ++index)
            this.AddEntity<MineCart.CosmeticFallingBoulder>(new MineCart.CosmeticFallingBoulder((float) Game1.random.Next(72, 195), new Color(100, 66, 49), (float) (96 /*0x60*/ + Game1.random.Next(-10, 11)), (float) (0.64999997615814209 + (double) index * 0.05000000074505806))).position = new Vector2((float) (5 + Game1.random.Next(360)), -16f);
          break;
        case 9:
          for (int index = 0; index < 8; ++index)
            this.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(368, 784, 16 /*0x10*/, 16 /*0x10*/), new Vector2((float) (274 + Game1.random.Next(-19, 20)), (float) (46 + Game1.random.Next(6, 14))), (float) Game1.random.Next(-4, 5), -16f, gravity_multiplier: 0.05f, life_time: 2f, num_animation_frames: 3, animation_interval: 0.33f, draw_depth: 0.35f, holdLastFrame: true, timeBeforeDisplay: (float) (1.0 + (double) index * 0.10000000149011612))).SetStartSound("dirtyHit");
          break;
      }
  ...
  public void PlayLevelMusic()
  {
    ...
    {
      case 0:
        Game1.changeMusicTrack("EarthMine", music_context: MusicContext.MiniGame);
        break;
      case 1:
        Game1.changeMusicTrack("FrostMine", music_context: MusicContext.MiniGame);
        break;
      case 2:
        Game1.changeMusicTrack("junimoKart_whaleMusic", music_context: MusicContext.MiniGame);
        break;
      case 3:
        Game1.changeMusicTrack("junimoKart_ghostMusic", music_context: MusicContext.MiniGame);
        break;
      case 4:
        Game1.changeMusicTrack("tribal", music_context: MusicContext.MiniGame);
        break;
      case 5:
        Game1.changeMusicTrack("junimoKart_slimeMusic", music_context: MusicContext.MiniGame);
        break;
      case 6:
        Game1.changeMusicTrack("nightTime", music_context: MusicContext.MiniGame);
        break;
      case 8:
        Game1.changeMusicTrack("Upper_Ambient", music_context: MusicContext.MiniGame);
        break;
      case 9:
        Game1.changeMusicTrack("junimoKart_mushroomMusic", music_context: MusicContext.MiniGame);
        break;
    }
  }
  ...
  public void EndCutscene()
  {
    ...
    if (this.gameMode != 3 || this.currentTheme != 7)
      return;
    int num = (int) Game1.player.stats.Increment("completedJunimoKart");
  ...
  {
    for (int forceXPosition = 0; forceXPosition < 16 /*0x10*/; ++forceXPosition)
  ...
  public void AddValidObstacle(MineCart.ObstacleTypes obstacle_type, Type type)
  {
    if (this._validObstacles == null)
      return;
    List<Type> typeList;
    if (!this._validObstacles.TryGetValue(obstacle_type, out typeList))
      this._validObstacles[obstacle_type] = typeList = new List<Type>();
    typeList.Add(type);
  }
  ...
  public void setUpTheme(int whichTheme)
  {
    ...
    this.midBGSource = new Rectangle(64 /*0x40*/, 0, 96 /*0x60*/, 162);
    this.backBGSource = new Rectangle(64 /*0x40*/, 162, 96 /*0x60*/, 111);
    this.lakeBGSource = new Rectangle(0, 80 /*0x50*/, 16 /*0x10*/, 97);
    this.backBGYOffset = this.tileSize * 2;
    this.midBGYOffset = 0;
    switch (whichTheme)
    {
      ...
      case 1:
        ...
        this.lakeBGSource = new Rectangle(304, 0, 16 /*0x10*/, 0);
        this.lakeTint = new Color(147, 217, (int) byte.MaxValue);
        this.midBGSource = new Rectangle(320, 135, 96 /*0x60*/, 149);
        this.midBGYOffset = -13;
        this.waterfallTint = Color.LightCyan * 0.5f;
        this.trackTint = new Color(186, 240 /*0xF0*/, (int) byte.MaxValue);
      ...
      case 2:
        ...
        this.trackShadowTint = new Color(32 /*0x20*/, 45, 180);
        this.midBGSource = new Rectangle(416, 0, 96 /*0x60*/, 69);
        this.backBGSource = new Rectangle(320, 0, 96 /*0x60*/, 135);
      ...
      case 3:
        ...
        this.lakeTint = new Color(60, 70, 80 /*0x50*/);
      ...
      case 4:
        ...
        this.lakeBGSource = new Rectangle(304, 97, 16 /*0x10*/, 97);
        this.trackTint = new Color((int) byte.MaxValue, 160 /*0xA0*/, 160 /*0xA0*/);
      ...
      case 5:
        ...
        this.midBGSource = new Rectangle(416, 69, 96 /*0x60*/, 162);
        this.midBGTint = Color.White;
      ...
      case 6:
        ...
        this.trackShadowTint = Color.Black;
        this.midBGSource = new Rectangle(416, 231, 96 /*0x60*/, 53);
        this.backBGSource = new Rectangle(320, 284, 96 /*0x60*/, 116);
      ...
      case 9:
        ...
        this.CreateBGDecor();
        this.backBGTint = Color.White;
        this.backBGSource = new Rectangle(0, 789, 96 /*0x60*/, 111);
        this.midBGTint = Color.White;
        this.caveTint = Color.Purple;
        this.lakeBGSource = new Rectangle(304, 0, 16 /*0x10*/, 0);
        this.lakeTint = new Color(0, 8, 46);
        this.midBGSource = new Rectangle(416, 736, 96 /*0x60*/, 149);
        this.midBGYOffset = -13;
  ...
  public T GetOverlap<T>(MineCart.ICollideable source) where T : MineCart.Entity
  {
    ...
    foreach (MineCart.Entity entity in this._entities)
    {
      if (entity.IsActive() && entity is MineCart.ICollideable collideable && entity is T overlap)
      {
  ...
  public List<T> GetOverlaps<T>(MineCart.ICollideable source) where T : MineCart.Entity
  {
    ...
    foreach (MineCart.Entity entity in this._entities)
    {
      if (entity.IsActive() && entity is MineCart.ICollideable collideable && entity is T obj)
      {
        Rectangle bounds2 = collideable.GetBounds();
        if (bounds1.Intersects(bounds2))
          overlaps.Add(obj);
  ...
  public void draw(SpriteBatch b)
  {
    ...
    {
      case MineCart.GameStates.FruitsSummary:
        SparklingText perfectText = this.perfectText;
        if (perfectText != null)
        {
          perfectText.draw(b, this.TransformDraw(new Vector2(80f, 40f)));
          goto case MineCart.GameStates.Map;
        }
        goto case MineCart.GameStates.Map;
      case MineCart.GameStates.Map:
      case MineCart.GameStates.Cutscene:
        ...
        {
          case MineCart.GameStates.Title:
            b.Draw(this.texture, this.TransformDraw(new Vector2((float) (this.screenWidth / 2 - 128 /*0x80*/), (float) (this.screenHeight / 2 - 35))), new Rectangle?(new Rectangle(256 /*0x0100*/, 409, 256 /*0x0100*/, 71)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.25f);
            if (this.gameMode == 2)
            {
              ...
              goto case MineCart.GameStates.FruitsSummary;
            }
            goto case MineCart.GameStates.FruitsSummary;
          case MineCart.GameStates.FruitsSummary:
            if (!this.gamePaused && (this.gameState == MineCart.GameStates.Ingame || this.gameState == MineCart.GameStates.Cutscene || this.gameState == MineCart.GameStates.FruitsSummary || this.gameState == MineCart.GameStates.Map))
            {
              ...
              else
              {
                ...
                {
                  b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(160 /*0xA0*/, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.07f);
                  b.Draw(this.texture, this.TransformDraw(dest + new Vector2(1f, 1f)), new Rectangle?(new Rectangle(160 /*0xA0*/, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.071f);
                  dest.X += 18f;
                ...
                {
                  b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(160 /*0xA0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.07f);
                  b.Draw(this.texture, this.TransformDraw(dest + new Vector2(1f, 1f)), new Rectangle?(new Rectangle(160 /*0xA0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.071f);
                  dest.X -= 18f;
                }
              }
              dest.X = 4f;
              ...
              for (int index = 0; index < 3; ++index)
              {
                ...
                {
                  b.Draw(this.texture, this.TransformDraw(dest + zero), new Rectangle?(new Rectangle(160 /*0xA0*/ + index * 16 /*0x10*/, 0, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.07f);
                  b.Draw(this.texture, this.TransformDraw(dest + new Vector2(1f, 1f) + zero), new Rectangle?(new Rectangle(160 /*0xA0*/ + index * 16 /*0x10*/, 0, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.075f);
                }
                else
                {
                  b.Draw(this.texture, this.TransformDraw(dest + zero), new Rectangle?(new Rectangle(160 /*0xA0*/ + index * 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.07f);
                  b.Draw(this.texture, this.TransformDraw(dest + zero + new Vector2(1f, 1f)), new Rectangle?(new Rectangle(160 /*0xA0*/ + index * 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(0.0f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.075f);
                }
                dest.X += 18f;
              }
              if (this.gameMode == 3)
              ...
              if (this.gameState == MineCart.GameStates.Ingame)
              {
                float num2 = (float) (this.screenWidth - 192 /*0xC0*/) / 2f;
                float b1 = num2 + 192f;
                dest = new Vector2(num2, 4f);
                for (int index = 0; index < 12; ++index)
                {
                  Rectangle rectangle = new Rectangle(192 /*0xC0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/);
                  if (index == 0)
                    rectangle = new Rectangle(176 /*0xB0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/);
                  else if (index >= 11)
                    rectangle = new Rectangle(207, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/);
                ...
                b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(176 /*0xB0*/, 64 /*0x40*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.15f);
                dest.X += 8f;
                ...
                if (this.player != null && this.player.visible)
                  dest.X = Utility.Lerp(num2, b1, Math.Min(this.player.position.X / (float) (this.distanceToTravel * this.tileSize), 1f));
                b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(240 /*0xF0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.12f);
                b.Draw(this.texture, this.TransformDraw(dest + new Vector2(1f, 1f)), new Rectangle?(new Rectangle(240 /*0xF0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(8f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.13f);
                if ((double) this.checkpointPosition > (double) this.tileSize * 0.5)
                {
                  dest.X = Utility.Lerp(num2, b1, this.checkpointPosition / (float) (this.distanceToTravel * this.tileSize));
                  b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(224 /*0xE0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.125f);
                  b.Draw(this.texture, this.TransformDraw(dest + new Vector2(1f, 1f)), new Rectangle?(new Rectangle(224 /*0xE0*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/)), Color.Black, 0.0f, new Vector2(8f, 0.0f), this.GetPixelScale(), SpriteEffects.None, 0.135f);
                }
              }
            }
            ...
            if (!Game1.options.hardwareCursor && !Game1.options.gamepadControls)
              b.Draw(Game1.mouseCursors, new Vector2((float) Game1.getMouseX(), (float) Game1.getMouseY()), new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, Game1.options.gamepadControls ? 44 : 0, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, Vector2.Zero, (float) (4.0 + (double) Game1.dialogueButtonScale / 150.0), SpriteEffects.None, 0.0001f);
            b.End();
            Game1.isUsingBackToFrontSorting = false;
            b.GraphicsDevice.ScissorRectangle = scissorRectangle;
            return;
          case MineCart.GameStates.Map:
            b.Draw(this.texture, this.TransformDraw(new Vector2(0.0f, 0.0f)), new Rectangle?(new Rectangle(0, 512 /*0x0200*/, 400, 224 /*0xE0*/)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.6f);
            if (!this.levelThemesFinishedThisRun.Contains(3))
              b.Draw(this.texture, this.TransformDraw(new Vector2(221f, 104f)), new Rectangle?(new Rectangle(455, 512 /*0x0200*/, 57, 64 /*0x40*/)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.21f);
            b.Draw(this.texture, this.TransformDraw(new Vector2(369f, 51f)), new Rectangle?(new Rectangle(480, 579, 31 /*0x1F*/, 32 /*0x20*/)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.21f);
            b.Draw(this.texture, this.TransformDraw(new Vector2(109f, 198f)), new Rectangle?(new Rectangle(420, 512 /*0x0200*/, 25, 26)), Color.White, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.21f);
            ...
            goto case MineCart.GameStates.FruitsSummary;
          case MineCart.GameStates.Cutscene:
            float scale = this.GetPixelScale() / 4f;
            b.DrawString(Game1.dialogueFont, this.cutsceneText, this.TransformDraw(new Vector2((float) (this.screenWidth / 2 - (int) ((double) Game1.dialogueFont.MeasureString(this.cutsceneText).X / 2.0 / 4.0)), 32f)), Color.White, 0.0f, Vector2.Zero, scale, SpriteEffects.None, 0.199f);
            goto case MineCart.GameStates.FruitsSummary;
          default:
            for (int index1 = 0; index1 < this.waterfallWidth; index1 += 2)
            {
              for (int index2 = -2; index2 <= this.screenHeight / this.tileSize + 1; ++index2)
                b.Draw(this.texture, this.TransformDraw(new Vector2((float) (this.screenWidth + this.tileSize * index1) - this.waterFallPosition, (float) (index2 * this.tileSize + (int) (this._totalTime * 48.0 + (double) (this.tileSize * 100)) % this.tileSize))), new Rectangle?(new Rectangle(48 /*0x30*/, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), this.waterfallTint, 0.0f, Vector2.Zero, this.GetPixelScale(), SpriteEffects.None, 0.2f);
            }
            goto case MineCart.GameStates.FruitsSummary;
        }
      default:
        for (int index = 0; index <= this.screenWidth / this.tileSize + 1; ++index)
          b.Draw(this.texture, this.TransformDraw(new Rectangle(index * this.tileSize - (int) this.lakeSpeedAccumulator % this.tileSize, this.tileSize * 9, this.tileSize, this.screenHeight - 96 /*0x60*/)), new Rectangle?(this.lakeBGSource), this.lakeTint, 0.0f, Vector2.Zero, SpriteEffects.None, 0.9f);
  ...
  public class MapJunimo : MineCart.Entity
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      Rectangle rectangle = new Rectangle(400, 512 /*0x0200*/, 16 /*0x10*/, 16 /*0x10*/);
      switch (this.direction)
      {
        case 0:
          rectangle.Y = 544;
  ...
  public class LakeDecor
  {
    ...
    {
      this._game = game;
      this._position = new Point(Game1.random.Next(0, this._game.screenWidth), Game1.random.Next(160 /*0xA0*/, this._game.screenHeight));
      if (forceXPosition != -1)
        this._position.X = forceXPosition * (this._game.screenWidth / 16 /*0x10*/) + Game1.random.Next(0, this._game.screenWidth / 16 /*0x10*/);
    ...
    public void Draw(SpriteBatch b)
    {
      ...
      float num1 = 32f;
      float t = (float) (this._position.Y - 160 /*0xA0*/) / (float) (this._game.screenHeight - 160 /*0xA0*/);
      ...
      if (this._bgDecor)
      {
        ...
        if (this._game.currentTheme == 3)
          y += (float) (int) (Math.Sin((double) Utility.Lerp(0.0f, 6.28318548f, (float) ((this._game.totalTimeMS + (double) (this._position.X * 7) + (double) (this._position.Y * 2)) / 2.0 % 1000.0) / 1000f)) * 3.0);
      }
      ...
      b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(vector2.X, y)), new Rectangle?(new Rectangle(96 /*0x60*/ + this.spriteIndex % 14 * this._game.tileSize + (int) ((this._game.totalTimeMS + (double) (this._position.X * 10)) % 1000.0 / (double) (1000 / this._animationFrames)) % 14 * this._game.tileSize, 848 + this.spriteIndex / 14 * this._game.tileSize, 16 /*0x10*/, 16 /*0x10*/)), this.spriteIndex == 0 ? this._game.midBGTint : (this.spriteIndex == 1 ? this._game.lakeTint : Color.White), 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, this._bgDecor ? 0.65f : (float) (0.800000011920929 + (double) t * (-1.0 / 1000.0)));
    }
  }

  public class StraightAwayGenerator(MineCart game) : MineCart.BaseTrackGenerator(game)
  {
      ...
      for (int index = 0; index < this.straightAwayLength; ++index)
      {
        ...
        if (this.currentStaggerDistance <= 0)
        {
          if (Game1.random.NextDouble() < (double) this.staggerChance)
            this._game.generatorPosition.Y += Game1.random.ChooseFrom<int>((IList<int>) this.staggerPattern);
  ...
  public class SmallGapGenerator(MineCart game) : MineCart.BaseTrackGenerator(game)
  {
    public int minLength = 3;
    public int maxLength = 5;
  ...
  public class RapidHopsGenerator(MineCart game) : MineCart.BaseTrackGenerator(game)
  {
    public int minLength = 3;
    public int maxLength = 5;
    private int startY;
  ...
  public class NoxiousMushroom : MineCart.Obstacle
  {
    ...
    {
      new Rectangle(288, 736, 16 /*0x10*/, 16 /*0x10*/),
      new Rectangle(288, 752, 16 /*0x10*/, 16 /*0x10*/),
      new Rectangle(288, 768 /*0x0300*/, 16 /*0x10*/, 16 /*0x10*/)
    };
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(this.frames[this.currentFrame]), Color.White, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.45f);
    }
  ...
  public class MushroomSpring : MineCart.Obstacle
  {
    ...
    public Rectangle[] frames = new Rectangle[3]
    {
      new Rectangle(400, 736, 16 /*0x10*/, 16 /*0x10*/),
      new Rectangle(400, 752, 16 /*0x10*/, 16 /*0x10*/),
      new Rectangle(400, 768 /*0x0300*/, 16 /*0x10*/, 16 /*0x10*/)
    };
    public int currentFrame;
    public float frameDuration = 0.05f;
    public float frameTimer;
    ...
    {
      this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(368, 784, 16 /*0x10*/, 16 /*0x10*/), Utility.PointToVector2(this.GetBounds().Center), (float) x, (float) y, 0.25f, 0.0f, 0.9f, num_animation_frames: 3, animation_interval: 0.3f));
    }
  ...
  public class MushroomBalanceTrackGenerator : MineCart.BaseTrackGenerator
  {
    ...
    protected override void _GenerateTrack()
    {
      ...
      {
        if (index1 != 1 || !Game1.random.NextBool())
        {
          ...
          if (index1 != 1)
          {
            if (index1 == 3 && Game1.random.NextBool())
              y1 -= 4;
            else if (this.staggerPattern != null && this.staggerPattern.Count > 0)
              y1 += Game1.random.ChooseFrom<int>((IList<int>) this.staggerPattern);
          }
          ...
          if (index1 == 0 || index1 == 2)
          {
            ...
            MineCart.BalanceTrack balanceTrack1 = new MineCart.BalanceTrack(MineCart.Track.TrackType.MushroomLeft, false);
            balanceTrack1.position.X = (float) (this._game.generatorPosition.X * this._game.tileSize);
            balanceTrack1.position.Y = this._game.trackBuilderCharacter.position.Y + num;
            balanceTrack1.startY = balanceTrack1.position.Y;
            this.AddTrack((MineCart.Track) balanceTrack1);
            collection2.Add(balanceTrack1);
            ++this._game.generatorPosition.X;
            MineCart.BalanceTrack balanceTrack2 = new MineCart.BalanceTrack(MineCart.Track.TrackType.MushroomMiddle, false);
            balanceTrack2.position.X = (float) (this._game.generatorPosition.X * this._game.tileSize);
            balanceTrack2.position.Y = this._game.trackBuilderCharacter.position.Y + num;
            balanceTrack2.startY = balanceTrack2.position.Y;
            this.AddTrack((MineCart.Track) balanceTrack2);
            collection2.Add(balanceTrack2);
            ++this._game.generatorPosition.X;
            MineCart.BalanceTrack balanceTrack3 = new MineCart.BalanceTrack(MineCart.Track.TrackType.MushroomRight, false);
            balanceTrack3.position.X = (float) (this._game.generatorPosition.X * this._game.tileSize);
            balanceTrack3.position.Y = this._game.trackBuilderCharacter.position.Y + num;
            balanceTrack3.startY = balanceTrack3.position.Y;
            this.AddTrack((MineCart.Track) balanceTrack3);
            collection2.Add(balanceTrack3);
            ++this._game.generatorPosition.X;
            foreach (MineCart.BalanceTrack balanceTrack4 in collection2)
              balanceTrack4.connectedTracks = new List<MineCart.BalanceTrack>((IEnumerable<MineCart.BalanceTrack>) collection2);
            if (index1 == 2)
            {
              foreach (MineCart.BalanceTrack balanceTrack5 in collection1)
                balanceTrack5.counterBalancedTracks = new List<MineCart.BalanceTrack>((IEnumerable<MineCart.BalanceTrack>) collection2);
              foreach (MineCart.BalanceTrack balanceTrack6 in collection2)
                balanceTrack6.counterBalancedTracks = new List<MineCart.BalanceTrack>((IEnumerable<MineCart.BalanceTrack>) collection1);
            }
  ...
  public class BaseTrackGenerator
  {
    ...
    {
      return (int) ((double) track.position.X / 16.0) % 2 == 0;
    }
    ...
    public void PopulateObstacles()
    {
      ...
      foreach (int key1 in this._obstacleIndices.Keys)
      {
        ...
        double chance = (double) obstacleIndex.Value;
        if (random.NextBool((float) chance))
        {
          int index;
          switch (key1)
          {
  ...
  public class Entity
  {
    ...
    {
      return (double) this.position.X >= (double) this._game.screenLeftBound - (double) (this._game.tileSize * 4) && (double) this.position.X <= (double) this._game.screenLeftBound + (double) this._game.screenWidth + (double) (this._game.tileSize * 4);
    }
  ...
  public class Bubble : MineCart.Obstacle
  {
    ...
    {
      this._normalizedVelocity.X = (float) Math.Cos((double) angle * 3.1415927410125732 / 180.0);
      this._normalizedVelocity.Y = -(float) Math.Sin((double) angle * 3.1415927410125732 / 180.0);
    ...
    public void Pop(bool play_sound = true)
    {
      ...
      this.Destroy();
      MineCart game = this._game;
      Rectangle source_rect = new Rectangle(32 /*0x20*/, 240 /*0xF0*/, 16 /*0x10*/, 16 /*0x10*/);
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + this.bubbleOffset), new Rectangle?(new Rectangle(this._frames[this._currentFrame] * 16 /*0x10*/, 256 /*0x0100*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.27f);
    }
  }
    ...
    protected override void _Update(float time)
    {
      ...
      else
      {
        if ((double) this.timer > 0.0)
          return;
        --this.bubbleCount;
        this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(0, 256 /*0x0100*/, 16 /*0x10*/, 16 /*0x10*/), this.position + new Vector2((float) (-(double) this._game.player.characterExtraHeight - 16.0)) / 2f, -10f, 10f, gravity_multiplier: -1f, life_time: 1.5f, scale: 0.5f, num_animation_frames: 4, holdLastFrame: true));
  ...
  public class Whale : MineCart.Entity
  {
    ...
    protected override void _Update(float time)
    {
      ...
      this.position.X = (float) ((double) this._game.screenLeftBound - 128.0 + (double) this._game.screenWidth + Math.Cos(this._game.totalTime * Math.PI / 2.2999999523162842) * 24.0);
      ...
      switch (this._currentState)
      {
        ...
        case MineCart.Whale.CurrentState.FireBubbles:
          ...
          if ((double) this._nextFire <= 0.0)
          {
            ...
            this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(0, 256 /*0x0100*/, 16 /*0x10*/, 16 /*0x10*/), this.position + new Vector2(96f, -100f), -10f, 10f, gravity_multiplier: -1f, life_time: 1f, scale: 0.5f, num_animation_frames: 4, animation_interval: 0.25f));
          }
          if ((double) this._stateTimer >= 0.0)
            break;
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + new Vector2(85f, 0.0f)), new Rectangle?(new Rectangle(86, 288, 75, 112 /*0x70*/)), Color.White, 0.0f, new Vector2(0.0f, 112f), this._game.GetPixelScale(), SpriteEffects.None, 0.29f);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + Utility.PointToVector2(p)), new Rectangle?(new Rectangle(point.X, 288 + point.Y, 85, 112 /*0x70*/)), Color.White, 0.0f, new Vector2(0.0f, 112f), this._game.GetPixelScale(), SpriteEffects.None, 0.28f);
    }
  ...
  public class EndingJunimo : MineCart.Entity
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(Game1.mouseCursors, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(294 + (int) (this._game.totalTimeMS % 400.0) / 100 * 16 /*0x10*/, 1432, 16 /*0x10*/, 16 /*0x10*/)), this._color, 0.0f, new Vector2(8f, 16f), (float) ((double) this._game.GetPixelScale() * 2.0 / 3.0), SpriteEffects.None, 0.25f);
    }
  }
  ...
  public class WillOWisp : MineCart.Obstacle
  {
    ...
    protected override void _Update(float time)
    {
      ...
      float num = 15f;
      this.offset.Y = (float) (Math.Sin((double) this._age * (double) num * 3.1415927410125732 / 180.0) - 1.0) * 32f;
      this.offset.X = (float) Math.Cos((double) this._age * (double) num * 3.0 * 3.1415927410125732 / 180.0) * 64f;
      this.offset.Y += (float) Math.Sin((double) this._age * (double) num * 6.0 * 3.1415927410125732 / 180.0) * 16f;
      ...
      this.scale = Utility.Lerp(0.5f, 0.6f, (float) Math.Sin((double) this._age * 200.0 * 3.1415927410125732 / 180.0) + 0.5f);
      this.nextDebris -= time;
      if ((double) this.nextDebris > 0.0)
        return;
      this.nextDebris = 0.1f;
      MineCart game = this._game;
      Rectangle source_rect = new Rectangle(192 /*0xC0*/, 96 /*0x60*/, 16 /*0x10*/, 16 /*0x10*/);
      Rectangle bounds = this.GetBounds();
      double x = (double) bounds.Center.X;
      bounds = this.GetBounds();
      ...
      double dx = (double) Game1.random.Next(-30, 31 /*0x1F*/);
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + this.offset), new Rectangle?(new Rectangle(192 /*0xC0*/, 80 /*0x50*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, (float) ((double) this._age * 200.0 * (Math.PI / 180.0)), new Vector2(8f, 8f), this._game.GetPixelScale() * this.scale, SpriteEffects.None, 0.27f);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + this.offset), new Rectangle?(new Rectangle(160 /*0xA0*/, 112 /*0x70*/, 32 /*0x20*/, 32 /*0x20*/)), Color.White, (float) ((double) this._age * 60.0 * (Math.PI / 180.0)), new Vector2(16f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.29f);
      if ((double) this._age <= 0.25)
        return;
      Vector2 vector2 = new Vector2(this.tailLength, this.scale);
      if ((double) this.tailLength <= 0.5)
        return;
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + this.offset), new Rectangle?(new Rectangle(208 /*0xD0*/ + (int) ((double) this._age / 0.10000000149011612) % 3 * 16 /*0x10*/, 80 /*0x50*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, this.tailRotation, new Vector2(16f, 8f), vector2 * this._game.GetPixelScale(), SpriteEffects.None, 0.44f);
    }
  }
  ...
  public class CosmeticFallingBoulder : MineCart.FallingBoulder
  {
    ...
    protected override void _Update(float time)
    {
      ...
      if ((double) this.position.Y >= (double) this.yBreakPosition)
      {
        ...
        {
          MineCart game = this._game;
          Rectangle source_rect = new Rectangle(16 /*0x10*/, 80 /*0x50*/, 16 /*0x10*/, 16 /*0x10*/);
          Rectangle bounds = this.GetBounds();
          double x = (double) bounds.Center.X;
          bounds = this.GetBounds();
          double bottom = (double) bounds.Bottom;
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      if (Math.Floor((double) this._age / 0.5) % 2.0 == 0.0)
        effects = SpriteEffects.FlipHorizontally;
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(0, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), this.color, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), effects, 0.15f);
    }
  }
  ...
  public class NoxiousGas : MineCart.Obstacle
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      if (Math.Floor((double) this._age / 0.5) % 2.0 == 0.0)
        effects = SpriteEffects.FlipHorizontally;
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(368, 784, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale() * Utility.Clamp(this._age / 0.5f, 0.0f, 1f), effects, 0.44f);
    }
  ...
  public class FallingBoulder : MineCart.Obstacle
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      if (Math.Floor((double) this._age / 0.5) % 2.0 == 0.0)
        effects = SpriteEffects.FlipHorizontally;
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(0, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), this._game.caveTint, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), effects, 0.45f);
    }
      ...
      if (this._tracks != null && this._tracks.Count > 0)
      {
        ...
        else if ((double) this.position.Y >= (double) this._tracks[0].GetYAtPoint(this.position.X))
        {
          ...
          if (this.IsOnScreen())
            Game1.playSound("hammer");
          for (int index = 0; index < 3; ++index)
            this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(16 /*0x10*/, 80 /*0x50*/, 16 /*0x10*/, 16 /*0x10*/), new Vector2((float) this.GetBounds().Center.X, (float) this.GetBounds().Bottom), (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-30, -19), 0.25f)).SetColor(this._game.caveTint);
        }
      }
      if ((double) this._currentFallSpeed < (double) this._fallSpeed)
      {
    ...
    {
      return (double) this.position.Y > (double) (this._game.screenHeight + 32 /*0x20*/) || base.ShouldReap();
    }
  }
  ...
  public class SlimeTrack : MineCart.Obstacle
  {
    ...
    {
      SpriteEffects effects = SpriteEffects.None;
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(0, 192 /*0xC0*/, 32 /*0x20*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), effects, 0.45f);
    }
  ...
  public class HugeSlime : MineCart.Obstacle
  {
    ...
    public override Rectangle GetLocalBounds() => new Rectangle(-40, -60, 80 /*0x50*/, 60);
    ...
    {
      SpriteEffects effects = SpriteEffects.None;
      Rectangle rectangle = new Rectangle(160 /*0xA0*/, 176 /*0xB0*/, 96 /*0x60*/, 80 /*0x50*/);
      switch (this._currentFrame)
      {
        case 0:
          rectangle = new Rectangle(160 /*0xA0*/, 176 /*0xB0*/, 96 /*0x60*/, 80 /*0x50*/);
          break;
        case 1:
          rectangle = new Rectangle(160 /*0xA0*/, 256 /*0x0100*/, 96 /*0x60*/, 80 /*0x50*/);
          break;
        case 2:
          rectangle = new Rectangle(160 /*0xA0*/, 336, 96 /*0x60*/, 64 /*0x40*/);
          break;
      }
    ...
    protected override void _Update(float time)
    {
      ...
      float num = (float) (this._game.screenHeight + 32 /*0x20*/);
      if (trackForXposition != null)
      {
      ...
      if (this._grounded)
      {
        ...
        if ((double) this._timeUntilHop <= 0.0)
        {
          ...
          for (int index = 0; index < 8; ++index)
            this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(192 /*0xC0*/, 112 /*0x70*/, 16 /*0x10*/, 16 /*0x10*/), new Vector2((float) this.GetBounds().Center.X, (float) this.GetBounds().Bottom) + new Vector2((float) Game1.random.Next(-32, 33), (float) Game1.random.Next(-32, 0)), (float) Game1.random.Next(-10, 11), (float) Game1.random.Next(-50, -29), 0.25f, 0.25f, 1f, num_animation_frames: 4, animation_interval: 0.25f, draw_depth: 0.46f));
        }
      ...
      else
      {
        ...
        if ((double) this.position.Y > (double) num && (double) this._yVelocity > 0.0)
        {
          ...
          for (int index = 0; index < 8; ++index)
            this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(192 /*0xC0*/, 112 /*0x70*/, 16 /*0x10*/, 16 /*0x10*/), new Vector2((float) this.GetBounds().Center.X, (float) this.GetBounds().Bottom) + new Vector2((float) Game1.random.Next(-32, 33), (float) Game1.random.Next(-32, 0)), (float) Game1.random.Next(-80, 81), (float) Game1.random.Next(-10, 1), 0.25f, 0.25f, 1f, num_animation_frames: 4, animation_interval: 0.25f, draw_depth: 0.46f));
  ...
  public class Roadblock : MineCart.Obstacle
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(16 /*0x10*/, 0, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.45f);
    }
    ...
    public override bool OnBump(MineCart.PlayerMineCartCharacter player)
    {
      ...
      this.ShootDebris(Game1.random.Next(10, 41), Game1.random.Next(-40, 0));
      this.ShootDebris(Game1.random.Next(5, 31 /*0x1F*/), Game1.random.Next(-60, -19));
      this.ShootDebris(Game1.random.Next(5, 31 /*0x1F*/), Game1.random.Next(-60, -19));
      Game1.playSound("woodWhack");
      this.Destroy();
      return false;
    ...
    public void ShootDebris(int x, int y)
    {
      this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(48 /*0x30*/, 48 /*0x30*/, 16 /*0x10*/, 16 /*0x10*/), Utility.PointToVector2(this.GetBounds().Center), (float) x, (float) y, 0.25f, life_time: 1f));
    }
  }
  ...
  public class Fruit : MineCart.Pickup
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(160 /*0xA0*/ + 16 /*0x10*/ * (int) this._fruitType, 0, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 8f), this._game.GetPixelScale(), SpriteEffects.None, 0.43f);
    }
  }
  ...
  public class Coin : MineCart.Pickup
  {
    ...
    protected override void _Update(float time)
    {
      ...
      if (this.collected)
      {
        ...
        if ((double) this.afterCollectionTimer > 0.40000000596046448)
          this.Destroy();
      }
      base._Update(time);
    }
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(9 * ((int) this._game.totalTimeMS % num / (num / 12)), 273, 9, 9)), Color.White * (float) (1.0 - (double) this.afterCollectionTimer / 0.40000000596046448), 0.0f, new Vector2(4f, 4f), this._game.GetPixelScale(), SpriteEffects.None, 0.45f);
    }
  ...
  public class Pickup : MineCart.Entity, MineCart.ICollideable
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, 0.0f, new Vector2(8f, 8f), this._game.GetPixelScale(), SpriteEffects.None, 0.45f);
    }
  ...
  public class Track : MineCart.Entity
  {
    ...
    public override void _Draw(SpriteBatch b)
    {
      ...
      {
        b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(192 /*0xC0*/, 144 /*0x90*/, 16 /*0x10*/, 32 /*0x20*/)), this._game.trackTint, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06));
        b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(160 /*0xA0*/ + (int) this.trackType * 16 /*0x10*/, 144 /*0x90*/, 16 /*0x10*/, 32 /*0x20*/)), Color.White, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06 - 9.9999997473787516E-05));
      }
      else if (this.trackType >= MineCart.Track.TrackType.MushroomLeft && this.trackType <= MineCart.Track.TrackType.MushroomRight)
      {
        if (this.GetType() == typeof (MineCart.Track))
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(304 + (int) (this.trackType - 6) * 16 /*0x10*/, 736, 16 /*0x10*/, 48 /*0x30*/)), Color.White, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06));
        else
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(352 + (int) (this.trackType - 6) * 16 /*0x10*/, 736, 16 /*0x10*/, 48 /*0x30*/)), Color.White, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06));
      }
        ...
        b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(256 /*0x0100*/ + (int) (this.trackType - 2) * 16 /*0x10*/, 144 /*0x90*/, 16 /*0x10*/, 32 /*0x20*/)), this._game.trackTint, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06));
      else
        b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, this.drawnPosition.Y - 32f)), new Rectangle?(new Rectangle(160 /*0xA0*/ + (int) this.trackType * 16 /*0x10*/, 144 /*0x90*/, 16 /*0x10*/, 32 /*0x20*/)), this._game.trackTint, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06));
      ...
      if (this.trackType == MineCart.Track.TrackType.MushroomMiddle)
      {
        ...
        {
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, y + 16f)), new Rectangle?(new Rectangle(320, 784, 16 /*0x10*/, 64 /*0x40*/)), Color.White, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06 + 0.0099999997764825821));
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, y + 16f)), new Rectangle?(new Rectangle(368, 784, 16 /*0x10*/, 64 /*0x40*/)), this._game.trackShadowTint * num, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06 + 0.004999999888241291));
          num += 0.1f;
        }
      }
      else
      {
        ...
        {
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, y)), new Rectangle?(this._game.currentTheme == 4 ? new Rectangle(16 /*0x10*/ + (flag ? 1 : 0) * 16 /*0x10*/, 160 /*0xA0*/, 16 /*0x10*/, 16 /*0x10*/) : new Rectangle(16 /*0x10*/ + (flag ? 1 : 0) * 16 /*0x10*/, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), this._game.trackTint, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06 + 0.0099999997764825821));
          b.Draw(this._game.texture, this._game.TransformDraw(new Vector2(this.drawnPosition.X, y)), new Rectangle?(this._game.currentTheme == 4 ? new Rectangle(16 /*0x10*/ + (flag ? 1 : 0) * 16 /*0x10*/, 160 /*0xA0*/, 16 /*0x10*/, 16 /*0x10*/) : new Rectangle(16 /*0x10*/ + (flag ? 1 : 0) * 16 /*0x10*/, 32 /*0x20*/, 16 /*0x10*/, 16 /*0x10*/)), this._game.trackShadowTint * num, 0.0f, Vector2.Zero, this._game.GetPixelScale(), SpriteEffects.None, (float) (0.5 + (double) this.drawnPosition.Y * 9.9999997473787516E-06 + 0.004999999888241291));
          num += 0.1f;
          flag = !flag;
        }
      }
    ...
    public int GetYAtPoint(float x)
    {
      ...
      switch (this.trackType)
      {
        ...
        case MineCart.Track.TrackType.SlimeUpSlope:
          return (int) ((double) this.position.Y - 2.0 - (double) num);
        default:
          return (int) ((double) this.position.Y - 2.0);
      }
    }
  ...
  public class PlayerMineCartCharacter : MineCart.MineCartCharacter, MineCart.ICollideable
  {
    ...
    protected override void _Update(float time)
    {
      ...
      if (this._grounded && this._game.respawnCounter <= 0)
      {
        if (this._game.minecartLoop.IsPaused && this._game.currentTheme != 7)
          this._game.minecartLoop.Resume();
        if (num != (int) ((double) this.position.X / (double) this._game.tileSize) && Game1.random.NextBool())
    ...
    public override void OnJump() => Game1.playSound("pickUpItem", new int?(200));
    ...
    public override void OnLand()
    {
      ...
      else
      {
        ...
        {
          Game1.playSound("slimeHit");
          bool flag = this.GetTrack().GetType() != typeof (MineCart.Track);
          for (int index = 0; index < 3; ++index)
            this._game.AddEntity<MineCart.MineDebris>(new MineCart.MineDebris(new Rectangle(362 + (flag ? 5 : 0), 802, 5, 4), this.position, (float) Game1.random.Next(-30, 31 /*0x1F*/), (float) Game1.random.Next(-50, -39), life_time: 0.75f, animation_interval: 1f, draw_depth: 0.15f));
          return;
        }
        Game1.playSound("parry");
      }
  ...
  public class CheckpointIndicator : MineCart.Entity
  {
    ...
    protected override void _Update(float time)
    {
      if (!this._activated)
        return;
      this.swayTimer += time * 6.28318548f;
    ...
    public override void _Draw(SpriteBatch b)
    {
      float rotation = (float) ((double) this.rotation * 3.1415927410125732 / 180.0);
      Vector2 vector2 = new Vector2(0.0f, -12f);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(16 /*0x10*/, 112 /*0x70*/, 16 /*0x10*/, 16 /*0x10*/)), this._game.trackTint, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.31f);
      if (this._activated)
        b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + vector2), new Rectangle?(new Rectangle(48 /*0x30*/, 112 /*0x70*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, rotation, new Vector2(8f, 16f) + vector2, this._game.GetPixelScale(), SpriteEffects.None, 0.3f);
      else
        b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + vector2), new Rectangle?(new Rectangle(32 /*0x20*/, 112 /*0x70*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, rotation, new Vector2(8f, 16f) + vector2, this._game.GetPixelScale(), SpriteEffects.None, 0.3f);
    }
  }
  ...
  public class GoalIndicator : MineCart.Entity
  {
    ...
    {
      float rotation = (float) ((double) this.rotation * 3.1415927410125732 / 180.0);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition), new Rectangle?(new Rectangle(16 /*0x10*/, 128 /*0x80*/, 16 /*0x10*/, 16 /*0x10*/)), this._game.trackTint, 0.0f, new Vector2(8f, 16f), this._game.GetPixelScale(), SpriteEffects.None, 0.31f);
      Vector2 vector2 = new Vector2(0.0f, -8f);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + vector2), new Rectangle?(new Rectangle(32 /*0x20*/, 128 /*0x80*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, rotation, new Vector2(8f, 16f) + vector2, this._game.GetPixelScale(), SpriteEffects.None, 0.3f);
    }
  }
  ...
  public class MineCartCharacter : MineCart.BaseCharacter
  {
    ...
    protected override void _Update(float time)
    {
      ...
      else
      {
        ...
        if ((this._grounded || (double) this.jumpGracePeriod > 0.0) && (double) this._jumpBuffer > 0.0 && this._game.isJumpPressed)
        {
          this._jumpBuffer = 0.0f;
          this.Jump();
        }
        ...
        if (grounded && currentTrackType != this.currentTrackType)
        {
    ...
    public override void _Draw(SpriteBatch b)
    {
      if (this._game.respawnCounter / 200 % 2 != 0)
        return;
      float num = (float) ((double) this.rotation * 3.1415927410125732 / 180.0);
      ...
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + vector2_2 * -this.minecartBumpOffset + vector2_2 * 4f), new Rectangle?(new Rectangle(0, 0, 16 /*0x10*/, 16 /*0x10*/)), Color.White, num, new Vector2(8f, 14f), this.cartScale * this._game.GetPixelScale(), SpriteEffects.None, 0.45f);
      b.Draw(this._game.texture, this._game.TransformDraw(this.drawnPosition + vector2_2 * -this.minecartBumpOffset + vector2_2 * 4f), new Rectangle?(new Rectangle(0, 16 /*0x10*/, 16 /*0x10*/, 16 /*0x10*/)), Color.White, num, new Vector2(8f, 14f), this.cartScale * this._game.GetPixelScale(), SpriteEffects.None, 0.4f);
      b.Draw(Game1.mouseCursors, this._game.TransformDraw(this.drawnPosition + vector2_1 * -2f + vector2_2 * -this.minecartBumpOffset + vector2_2 * 12f + new Vector2(0.0f, -this.characterExtraHeight)), new Rectangle?(new Rectangle(294 + (int) (this._game.totalTimeMS % 400.0) / 100 * 16 /*0x10*/, 1432, 16 /*0x10*/, 16 /*0x10*/)), Color.Lime, 0.0f, new Vector2(8f, 8f), (float) ((double) this._game.GetPixelScale() * 2.0 / 3.0), SpriteEffects.None, 0.425f);
    }
  }
}
