...
public class Darts : IMinigame
{
  ...
  public bool tick(GameTime time)
  {
    ...
    switch (this.currentGameState)
    {
      ...
      case Darts.GameState.Charging:
        if (Darts.chargeSound != null)
          Game1.sounds.SetPitch(Darts.chargeSound, (float) (2400.0 * (1.0 - (double) this.chargeTime)));
      ...
      case Darts.GameState.Firing:
        if ((double) this.hangTime > 0.0)
        {
          ...
          if ((double) this.hangTime <= 0.0)
          {
            float num = Utility.RandomFloat(0.0f, 6.28318548f);
  ...
  {
    return this.currentGameState == Darts.GameState.Aiming && (double) this.cursorPosition.X > 0.0 && (double) this.cursorPosition.X < 320.0 && (double) this.cursorPosition.Y > 0.0 && (double) this.cursorPosition.Y < 320.0;
  }
  ...
  public void QuitGame()
  {
    ...
    if (this.points == 0)
    {
      ...
      if (this.startingDartCount == 20 && droppedLimitedNutCount == 0 || this.startingDartCount == 15 && droppedLimitedNutCount == 1 || this.startingDartCount == 10 && droppedLimitedNutCount == 2)
      {
        ...
        Game1.afterDialogues += (Game1.afterFadeFunction) (() => Game1.player.team.RequestLimitedNutDrops(nameof (Darts), Game1.currentLocation, 1984, 512 /*0x0200*/, 3));
      }
      else
  ...
  public void draw(SpriteBatch b)
  {
    ...
    if (this.IsAiming() || this.currentGameState == Darts.GameState.Charging)
      b.Draw(this.texture, this.TransformDraw(this.aimPosition), new Rectangle?(new Rectangle(0, 320, 64 /*0x40*/, 64 /*0x40*/)), Color.White * 0.5f, 0.0f, new Vector2(32f, 32f), this.GetPixelScale() * this.GetRadiusFromCharge(), SpriteEffects.None, 0.0f);
    if ((double) this.dartTime >= 0.0)
    {
      Rectangle rectangle = new Rectangle(0, 384, 16 /*0x10*/, 32 /*0x20*/);
      if ((double) this.dartTime > 0.64999997615814209)
        rectangle.X = 16 /*0x10*/;
      if ((double) this.dartTime > 0.89999997615814209)
        rectangle.X = 32 /*0x20*/;
    ...
    if (this.alternateTextString != "")
      SpriteText.drawStringWithScrollCenteredAt(b, this.alternateTextString, (int) ((double) vector2_1.X + (double) vector2_2.X), (int) ((double) vector2_1.Y + (double) vector2_2.Y), color: new Color?(SpriteText.color_Red));
    else if (this.points >= 0)
    {
      ...
      if (this.currentGameState == Darts.GameState.ShowScore || this.currentGameState == Darts.GameState.Scoring)
      {
        if (this.shakeScore)
          vector2_2 = new Vector2((float) Game1.random.Next(-1, 2), (float) Game1.random.Next(-1, 2));
        vector2_1.Y += 64f;
        string s2 = this.currentGameState == Darts.GameState.ShowScore ? $" {this.lastHitString} " : $" {this.lastHitAmount.ToString()} ";
        SpriteText.drawStringWithScrollCenteredAt(b, s2, (int) ((double) vector2_1.X + (double) vector2_2.X), (int) ((double) vector2_1.Y + (double) vector2_2.Y), color: new Color?(SpriteText.color_Blue), scrollType: 2);
      }
    }
    for (int index = 0; index < this.dartCount; ++index)
    {
      Vector2 dest = new Vector2((float) (7 + index * 10), 317f);
      b.Draw(this.texture, this.TransformDraw(dest), new Rectangle?(new Rectangle(64 /*0x40*/, 384, 16 /*0x10*/, 32 /*0x20*/)), Color.White, 0.0f, new Vector2(0.0f, 32f), this.GetPixelScale(), SpriteEffects.None, 0.02f);
    }
    if (this.gameOverString != "")
    {
      ...
      if (this.points == 0)
      {
        ...
        if (this.IsPerfectVictory())
          SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinTextPerfect", (object) this.throwsCount), (int) ((double) vector2_4.X + (double) vector2_2.X), (int) ((double) vector2_4.Y + (double) vector2_2.Y), color: new Color?(SpriteText.color_Blue), scrollType: 2);
        else
          SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinText", (object) this.throwsCount), (int) ((double) vector2_4.X + (double) vector2_2.X), (int) ((double) vector2_4.Y + (double) vector2_2.Y), color: new Color?(SpriteText.color_Blue), scrollType: 2);
      }
      else
      {
    ...
    if (Game1.options.gamepadControls && !Game1.options.hardwareCursor)
      b.Draw(Game1.mouseCursors, new Vector2((float) Game1.getMouseX(), (float) Game1.getMouseY()), new Rectangle?(Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, !Game1.options.snappyMenus || !Game1.options.gamepadControls ? 0 : 44, 16 /*0x10*/, 16 /*0x10*/)), Color.White * Game1.mouseCursorTransparency, 0.0f, Vector2.Zero, (float) (4.0 + (double) Game1.dialogueButtonScale / 150.0), SpriteEffects.None, 1f);
    b.End();
  }
