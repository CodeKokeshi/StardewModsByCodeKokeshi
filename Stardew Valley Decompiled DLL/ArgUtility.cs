...
public static class ArgUtility
{
  ...
  {
    return value == null ? defaultValue : ArgUtility.Get(value.Split(' ', index + 2, StringSplitOptions.RemoveEmptyEntries), index, defaultValue);
  }
  ...
  /// </remarks>
  public static string[] SplitQuoteAware(
    string input,
    char delimiter,
    StringSplitOptions splitOptions = StringSplitOptions.None,
    bool keepQuotesAndEscapes = false)
  {
    ...
    bool flag1 = false;
    if (splitOptions.HasFlag((Enum) StringSplitOptions.TrimEntries))
    {
      flag1 = true;
      splitOptions &= ~StringSplitOptions.TrimEntries;
    }
    bool flag2 = splitOptions.HasFlag((Enum) StringSplitOptions.RemoveEmptyEntries);
    string[] strArray1 = input.Split('"');
    List<string> stringList = new List<string>(strArray1.Length * 4);
    bool flag3 = true;
    bool flag4 = true;
    string str1 = (string) null;
    int index1 = 0;
    for (int index2 = strArray1.Length - 1; index1 <= index2; ++index1)
    {
      flag3 = !flag3;
      string str2 = strArray1[index1];
      bool flag5 = false;
      bool flag6 = false;
      bool flag7 = str2.EndsWith(delimiter);
      if (keepQuotesAndEscapes && index1 != 0)
        str2 = "\"" + str2;
      if (!flag4)
      {
        if (str1.EndsWith('\\'))
        {
          str2 = keepQuotesAndEscapes ? str1 + str2 : $"{str1.Substring(0, str1.Length - 1)}\"{str2}";
          flag3 = !flag3;
          flag5 = true;
        }
        else if (flag3 || !str2.StartsWith(delimiter))
          flag6 = true;
        else
      ...
      {
        if (flag5)
          stringList[stringList.Count - 1] = str2;
        else if (flag6)
        {
          stringList[stringList.Count - 1] += str2;
          str2 = stringList[stringList.Count - 1];
        }
        else
          stringList.Add(str2);
        str1 = str2;
      ...
      {
        if (flag7 && !flag2 && index1 != index2 && str2.Length > 0)
          str2 = str2.Substring(0, str2.Length - 1);
        string[] strArray2 = str2.Split(delimiter, splitOptions);
        switch (strArray2.Length)
        {
          case 0:
            ...
            break;
          case 1:
            if (flag7 && strArray2[0] == string.Empty)
            {
              str1 = string.Empty;
              break;
            }
          ...
          default:
            if (flag5)
            {
              ...
              stringList.AddRange((IEnumerable<string>) strArray2);
            }
            else if (flag6)
            {
              stringList[stringList.Count - 1] += strArray2[0];
              if (strArray2.Length > 1)
                stringList.AddRange((IEnumerable<string>) new ArraySegment<string>(strArray2, 1, strArray2.Length - 1));
            }
            else
              stringList.AddRange((IEnumerable<string>) strArray2);
    ...
    if (flag1)
    {
      ...
      {
        stringList[index3] = stringList[index3].Trim();
        if (flag2 && stringList[index3].Length == 0)
          stringList.RemoveAt(index3);
      }
    }
  ...
  public static string UnsplitQuoteAware(string[] input, char delimiter, int startAt = 0, int count = 2147483647 /*0x7FFFFFFF*/)
  {
    if (startAt < 0)
      throw new ArgumentException("Can't start unsplitting before the bounds of the array.", nameof (startAt));
    if (input == null || count == 0 || startAt >= input.Length)
      return string.Empty;
    count = Math.Min(count, input.Length - startAt);
    string[] strArray = new string[count];
    int index1 = startAt;
    for (int index2 = startAt + count - 1; index1 <= index2; ++index1)
    {
      string input1 = input[index1];
      if (input1.Contains('"'))
        input1 = ArgUtility.EscapeQuotes(input1);
      if (input1.Contains(delimiter))
        input1 = $"\"{input1}\"";
      strArray[index1 - startAt] = input1;
    }
  ...
  public static string EscapeQuotes(string input) => input.Replace("\"", "\\\"");
  ...
  {
    if (startAt < 0)
      throw new ArgumentException("Can't start copying before the bounds of the array.", nameof (startAt));
    if (array == null || length == 0 || startAt > array.Length - 1)
      return LegacyShims.EmptyArray<T>();
    if (startAt == 0 && (length == -1 || length == array.Length))
      return ((IEnumerable<T>) array).ToArray<T>();
    if (length < 0)
      length = array.Length - startAt;
    T[] destinationArray = new T[length];
    Array.Copy((Array) array, startAt, (Array) destinationArray, 0, length);
  ...
  public static string Get(string[] array, int index, string defaultValue = null, bool allowBlank = true)
  {
    if (index >= 0)
    {
      ...
      if (num < valueOrDefault & length.HasValue)
      {
        ...
        if (allowBlank || !string.IsNullOrWhiteSpace(str))
          return str;
      }
    }
    return defaultValue;
  ...
  public static bool TryGetOptional(
    string[] array,
    int index,
    out string value,
    out string error,
  ...
  {
    bool result;
    return !bool.TryParse(ArgUtility.Get(array, index), out result) ? defaultValue : result;
  }
  ...
  {
    string str;
    if (!ArgUtility.TryGet(array, index, out str, out error, false, name))
    {
      value = false;
      return false;
    }
  ...
  public static bool TryGetOptionalBool(
    string[] array,
    int index,
    out bool value,
    out string error,
  ...
  {
    int parsed;
    return !Utility.TryParseDirection(ArgUtility.Get(array, index), out parsed) ? defaultValue : parsed;
  }
  ...
  {
    string direction;
    if (!ArgUtility.TryGet(array, index, out direction, out error, false, name))
    {
      value = 0;
      return false;
    }
    if (!Utility.TryParseDirection(direction, out value))
    {
      value = 0;
  ...
  /// <inheritdoc cref="M:StardewValley.ArgUtility.TryGetOptional(System.String[],System.Int32,System.String@,System.String@,System.String,System.Boolean,System.String)" />
  public static bool TryGetOptionalDirection(
    string[] array,
    int index,
    out int value,
    out string error,
  ...
  {
    TEnum parsed;
    return !Utility.TryParseEnum<TEnum>(ArgUtility.Get(array, index), out parsed) ? defaultValue : parsed;
  }
    ...
    where TEnum : struct
  {
    ...
    if (!Utility.TryParseEnum<TEnum>(str, out value))
    {
      Type type = typeof (TEnum);
      value = default (TEnum);
      error = ArgUtility.GetValueParseError(array, index, name, true, $"an enum of type '{type.FullName ?? type.Name}' (should be one of {string.Join(", ", Enum.GetNames(typeof (TEnum)))})");
      return false;
    }
    error = (string) null;
    return true;
  ...
  /// <inheritdoc cref="M:StardewValley.ArgUtility.Get(System.String[],System.Int32,System.String,System.Boolean)" />
  public static float GetFloat(string[] array, int index, float defaultValue = 0.0f)
  {
    float result;
    return !float.TryParse(ArgUtility.Get(array, index), out result) ? defaultValue : result;
  }
  ...
  {
    string s;
    if (!ArgUtility.TryGet(array, index, out s, out error, false, name))
    {
      value = 0.0f;
      return false;
    }
  ...
  {
    int result;
    return !int.TryParse(ArgUtility.Get(array, index), out result) ? defaultValue : result;
  }
    ...
    [CallerArgumentExpression("value")] string name = null)
  {
    int x;
    int y;
    if (!ArgUtility.TryGetInt(array, index, out x, out error, name != null ? name + " > x" : (string) null) || !ArgUtility.TryGetInt(array, index + 1, out y, out error, name != null ? name + " > y" : (string) null))
    {
      value = Point.Zero;
      return false;
    }
  ...
  /// <inheritdoc cref="M:StardewValley.ArgUtility.TryGet(System.String[],System.Int32,System.String@,System.String@,System.Boolean,System.String)" />
  public static bool TryGetRectangle(
    string[] array,
    int index,
    out Rectangle value,
    out string error,
    [CallerArgumentExpression("value")] string name = null)
  {
    ...
    int width;
    int height;
    if (!ArgUtility.TryGetInt(array, index, out x, out error, name != null ? name + " > x" : (string) null) || !ArgUtility.TryGetInt(array, index + 1, out y, out error, name != null ? name + " > y" : (string) null) || !ArgUtility.TryGetInt(array, index + 2, out width, out error, name != null ? name + " > width" : (string) null) || !ArgUtility.TryGetInt(array, index + 3, out height, out error, name != null ? name + " > height" : (string) null))
    {
  ...
  public static bool TryGetVector2(
    ...
    [CallerArgumentExpression("value")] string name = null)
  {
    string name1 = name != null ? name + " > x" : (string) null;
    string name2 = name != null ? name + " > y" : (string) null;
    if (integerOnly)
    {
      int x;
      int y;
      if (ArgUtility.TryGetInt(array, index, out x, out error, name1) && ArgUtility.TryGetInt(array, index + 1, out y, out error, name2))
      {
    ...
    else
    {
      float x;
      float y;
      if (ArgUtility.TryGetFloat(array, index, out x, out error, name1) && ArgUtility.TryGetFloat(array, index + 1, out y, out error, name2))
      {
    ...
    char delimiter = ' ')
  {
    if (array == null || index < 0 || index >= array.Length)
      return defaultValue;
    return array.Length - index == 1 ? array[index] : string.Join(delimiter, RuntimeHelpers.GetSubArray<string>(array, Range.StartAt((Index) index)));
  }
  ...
  public static bool TryGetRemainder(
    string[] array,
    int index,
    out string value,
    out string error,
    ...
    [CallerArgumentExpression("value")] string name = null)
  {
    ...
    value = array.Length - index != 1 ? string.Join(delimiter, RuntimeHelpers.GetSubArray<string>(array, Range.StartAt((Index) index))) : array[index];
    error = (string) null;
    return true;
  }
  ...
  public static bool TryGetOptionalRemainder(
    string[] array,
    int index,
    out string value,
    string defaultValue = null,
  ...
  internal static string GetMissingRequiredIndexError(string[] array, int index, string name)
  {
    switch (array.Length)
    {
      ...
      default:
        return $"required {ArgUtility.GetFieldLabel(index, name)} not found (list has indexes 0 through {array.Length - 1})";
    }
  }
  ...
  /// <param name="name">The readable name for the argument being parsed, if available.</param>
  internal static string GetValueParseError(
    string[] array,
    int index,
    string name,
    bool required,
    string typeSummary)
  {
    return $"{(required ? nameof (required) : "optional")} {ArgUtility.GetFieldLabel(index, name)} has value '{array[index]}', which can't be parsed as {typeSummary}";
  }
