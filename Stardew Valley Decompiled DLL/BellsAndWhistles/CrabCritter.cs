  ...
  public override bool update(GameTime time, GameLocation environment)
  {
    ...
    if (this.skittering)
    {
      if ((double) this.yJumpOffset >= 0.0)
      {
        ...
        {
          if (environment.isWaterTile((int) this.position.X / 64 /*0x40*/, (int) this.position.Y / 64 /*0x40*/))
          {
    ...
    else
    {
      ...
      if (!this.skittering)
      {
        ...
        if ((double) this.nextMovementChange <= 0.0)
        {
          ...
          if (this.moving)
            this.movementDirection.X = Game1.random.NextBool() ? -1f : 1f;
          else
    ...
    else if (!this.diving && environment.isWaterTile((int) ((double) this.position.X / 64.0 + (double) Math.Sign(this.movementDirection.X) * 1.0), (int) this.position.Y / 64 /*0x40*/))
    {
      if ((double) this.yJumpOffset >= 0.0)
