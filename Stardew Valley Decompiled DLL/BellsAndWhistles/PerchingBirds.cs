...
public class PerchingBirds
{
    ...
    foreach (Point currentBirdLocation in this.GetCurrentBirdLocationList())
    {
      if (this._birdPointOccupancy[currentBirdLocation] == null)
      {
        ...
        if (bird != null)
        {
          ...
          {
            if ((double) Utility.distance(farmer.position.X, (float) (currentBirdLocation.X * 64 /*0x40*/) + 32f, farmer.position.Y, (float) (currentBirdLocation.Y * 64 /*0x40*/) + 32f) < 200.0)
              flag = true;
          }
        }
        if (!flag)
          options.Add(currentBirdLocation);
      }
    }
    return Game1.random.ChooseFrom<Point>((IList<Point>) options);
  }
  ...
  {
    return this.ShouldBirdsRoost() ? this._birdRoostLocations : this._birdLocations;
  }
