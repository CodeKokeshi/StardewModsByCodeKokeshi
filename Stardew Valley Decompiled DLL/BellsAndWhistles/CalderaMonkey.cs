...
public class CalderaMonkey : Critter
{
    ...
    switch (this.currentPhase)
    {
      ...
      case 1:
        if ((double) this.nextFrameTimer <= 0.0)
        {
          ...
          environment.TemporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 0, 64 /*0x40*/, 64 /*0x40*/), 150f, 3, 0, this.position + new Vector2(this.currentFrame == 2 ? 32f : -8f, 48f), false, Game1.random.NextBool(), 1f / 1000f, 0.02f, Color.White, 0.75f, 3f / 1000f, 0.0f, 0.0f));
          break;
        }
        break;
      case 2:
  ...
  public override void draw(SpriteBatch b)
  {
    ...
    for (int index = 0; index < this.buddies.Count; ++index)
    {
      ...
      b.Draw(Game1.staminaRect, new Rectangle((int) local.X + (int) y + 8, (int) local.Y + 44 + 2, 56 - (int) y * 2 - 16 /*0x10*/, 4), new Rectangle?(Game1.staminaRect.Bounds), new Color((int) byte.MaxValue, (int) byte.MaxValue, 150) * 0.55f, 0.0f, Vector2.Zero, SpriteEffects.None, (float) ((double) this.buddies[index].Y / 10000.0 + 1.0 / 1000.0));
      ...
      Rectangle? sourceRectangle = new Rectangle?(new Rectangle((int) totalGameTime.TotalMilliseconds % 700 / 70 * 16 /*0x10*/, 0, 16 /*0x10*/, 16 /*0x10*/));
