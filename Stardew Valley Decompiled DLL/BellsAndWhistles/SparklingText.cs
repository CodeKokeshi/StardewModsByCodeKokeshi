...
{
  public static int maxDistanceForSparkle = 32 /*0x20*/;
    ...
    float depth = 1f)
  {
    if (amplitude == -1)
      amplitude = 64 /*0x40*/;
    SparklingText.maxDistanceForSparkle = 32 /*0x20*/;
    this.font = font;
    this.color = color;
  ...
  public bool update(GameTime time)
  {
    ...
    if (this.millisecondsDuration > 500 && Game1.random.NextDouble() < this.sparkleFrequency)
      this.sparkles.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 704, 64 /*0x40*/, 64 /*0x40*/), (float) (Game1.random.Next(100, 600) / 6), 6, 0, new Vector2((float) Game1.random.Next(this.boundingBox.X, this.boundingBox.Right), (float) Game1.random.Next(this.boundingBox.Y, this.boundingBox.Bottom)), false, false, this.layerDepth, 0.0f, this.rainbow ? this.color : this.sparkleColor, 1f, 0.0f, 0.0f, 0.0f));
