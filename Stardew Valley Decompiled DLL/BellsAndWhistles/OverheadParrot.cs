...
public class OverheadParrot : Critter
{
  ...
  public override bool update(GameTime time, GameLocation environment)
  {
    ...
    if (this.currentFlapIndex == 4 && this.flapFrameAccumulator == 0 && Utility.isOnScreen(drawPosition, 64 /*0x40*/))
    ...
    if (!Game1.currentLocation.hasTileAt((int) shadowPosition.X / 64 /*0x40*/, (int) shadowPosition.Y / 64 /*0x40*/, "Back"))
      this._shouldDrawShadow = false;
    return (double) this.position.X < -64.0 - (double) this.swayAmount.X * 4.0 || (double) this.position.Y > (double) (environment.map.Layers[0].DisplayHeight + 64 /*0x40*/) + ((double) this.height + (double) this.swayAmount.Y) * 4.0;
  }
  ...
  {
    b.Draw(this._texture, Game1.GlobalToLocal(Game1.viewport, this.GetDrawPosition()), new Rectangle?(this.sourceRect), Color.White, 0.0f, new Vector2(12f, 20f), 4f, SpriteEffects.None, this.position.Y / 10000f);
  }
}
