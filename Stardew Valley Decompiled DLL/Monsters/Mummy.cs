...
public class Mummy : Monster
{
  ...
  {
    base.initNetFields();
    this.NetFields.AddField((INetSerializable) this.crumbleEvent, "crumbleEvent").AddField((INetSerializable) this.reviveTimer, "reviveTimer");
    ...
    Farmer who)
  {
    ...
    else
    {
      ...
      if (this.Health <= 0)
      {
        if (!isBomb && who.CurrentTool is MeleeWeapon currentTool && currentTool.hasEnchantmentOfType<CrusaderEnchantment>())
        {
          Utility.makeTemporarySpriteJuicier(new TemporaryAnimatedSprite(44, this.Position, Color.BlueViolet, 10)
          {
            holdLastFrame = true,
  ...
  private List<FarmerSprite.AnimationFrame> getCrumbleAnimation(bool reverse = false)
  {
    ...
    if (!reverse)
      crumbleAnimation.Add(new FarmerSprite.AnimationFrame(16 /*0x10*/, 100, 0, false, false));
    else
      crumbleAnimation.Add(new FarmerSprite.AnimationFrame(16 /*0x10*/, 100, 0, false, false, new AnimatedSprite.endOfAnimationBehavior(this.behaviorAfterRevival), true));
  ...
  protected override void updateAnimation(GameTime time)
  {
    ...
    else if (this.reviveTimer.Value > 0)
    {
      ...
      if (this.reviveTimer.Value < 2000)
        this.shake(this.reviveTimer.Value);
      if (this.reviveTimer.Value <= 0)
      {
        if (Game1.IsMasterGame)
        {
