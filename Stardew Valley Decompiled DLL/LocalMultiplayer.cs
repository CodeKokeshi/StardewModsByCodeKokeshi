...
public class LocalMultiplayer
{
  ...
  {
    return is_local_only ? Game1.hasLocalClientsOnly : GameRunner.instance.gameInstances.Count > 1;
  }
  ...
  private static void GetStaticFieldsAndDefaults()
  {
    ...
    foreach (Type element in typeList)
    {
      if (element.GetCustomAttribute<CompilerGeneratedAttribute>() == null)
      {
        bool flag = element.GetCustomAttribute<InstanceStatics>() != null;
        foreach (FieldInfo field in element.GetFields(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
        {
          if (!field.IsInitOnly && field.IsStatic && !field.IsLiteral && (flag || field.GetCustomAttribute<InstancedStatic>() != null) && field.GetCustomAttribute<NonInstancedStatic>() == null)
          {
