...
// Type: StardewValley.Network.NetEvents.SetMailRequest
...
public sealed class SetMailRequest : BaseSetFlagRequest
{
  ...
  public override void PerformAction(Farmer farmer)
  {
    ...
    {
      case MailType.Now:
        this.ToggleMailbox(farmer, this.FlagId, this.FlagState);
        break;
      case MailType.Tomorrow:
        farmer.mailForTomorrow.Toggle<string>(this.FlagId, this.FlagState);
        break;
      case MailType.Received:
        farmer.mailReceived.Toggle<string>(this.FlagId, this.FlagState);
        break;
      case MailType.All:
        this.ToggleMailbox(farmer, this.FlagId, this.FlagState);
        farmer.mailForTomorrow.Toggle<string>(this.FlagId, this.FlagState);
        ...
        break;
      default:
        Game1.log.Warn($"Received request to add mail ID '{this.FlagId}' with unknown mail type '{this.MailType}'");
        break;
    }
  }
  ...
  private void ToggleMailbox(Farmer farmer, string mailId, bool add)
  {
    if (add)
      ...
      farmer.mailbox.RemoveWhere((Func<string, bool>) (p => p == mailId));
  }
}
