...
// Type: StardewValley.ItemTypeDefinitions.ItemMetadata
...
public class ItemMetadata
{
  ...
  internal void SetTypeDefinition(
    string typeIdentifier,
    IItemDataDefinition typeDefinition,
    bool? itemExists = null)
  {
    this.TypeIdentifier = typeIdentifier;
    this.TypeDefinition = typeDefinition;
    this.IsTypeResolveAttempted = true;
    this.TypeDefinitionContainsItem = ((int) itemExists ?? (typeDefinition != null ? (typeDefinition.Exists(this.LocalItemId) ? 1 : 0) : 0)) != 0;
  }
  ...
  public IItemDataDefinition GetTypeDefinition()
  {
    if (!this.IsTypeResolveAttempted)
    {
      ...
      this.SetTypeDefinition(typeDefinitionFor?.Identifier ?? this.TypeIdentifier, typeDefinitionFor);
    }
  ...
  public ParsedItemData GetParsedData()
  {
    if (!this.IsParsedDataLoaded)
    {
      ...
      if (this.TypeDefinition != null)
      {
        ...
        this.ParsedData = (ParsedItemData) null;
  ...
  {
    return !this.Exists() ? (Item) null : ItemRegistry.Create(this.QualifiedItemId, amount, quality);
  }
