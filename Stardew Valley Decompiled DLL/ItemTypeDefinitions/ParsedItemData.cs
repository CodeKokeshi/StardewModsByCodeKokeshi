...
// Type: StardewValley.ItemTypeDefinitions.ParsedItemData
...
public class ParsedItemData : IHaveItemTypeId
{
  ...
  public ParsedItemData(
    ...
    bool excludeFromRandomSale = false)
  {
    string itemId1 = itemType.Identifier + itemId;
    if (string.IsNullOrWhiteSpace(internalName))
      internalName = itemId1;
    if (string.IsNullOrWhiteSpace(displayName))
      displayName = ItemRegistry.GetUnnamedItemName(itemId1);
    this.ItemType = itemType;
    this.ItemId = itemId;
    this.QualifiedItemId = itemId1;
  ...
  /// <inheritdoc />
  public string GetItemTypeId() => this.ItemType.Identifier;
    ...
    if (!this.IsErrorItem)
    {
      ...
      if (this.Texture != null)
      {
        ...
        if (offset == 0)
        {
          ...
          {
            nullable = spriteIndex;
            int spriteIndex1 = this.SpriteIndex;
            if (!(nullable.GetValueOrDefault() == spriteIndex1 & nullable.HasValue))
              goto label_5;
          }
          return this.DefaultSourceRect;
        }
label_5:
        IItemDataDefinition itemType = this.ItemType;
        Texture2D texture = this.Texture;
        nullable = spriteIndex;
        int spriteIndex2 = (nullable ?? this.SpriteIndex) + offset;
        return itemType.GetSourceRect(this, texture, spriteIndex2);
      }
    }
  ...
  public virtual bool HasCategory() => this.Category < -1;
