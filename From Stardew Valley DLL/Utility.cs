...
public class Utility
{
	...
	public static void ForEachCrop(Func<Crop, bool> action)
	{
		ForEachLocation(delegate(GameLocation location)
		{
			...
			foreach (StardewValley.Object value2 in location.objects.Values)
			{
	...
	{
		if (item.HasTypeObject() && item.GetType() == typeof(StardewValley.Object))
		{
	...
	public static float getFarmerItemsShippedPercent(Farmer who = null)
	{
		...
		foreach (ParsedItemData data in ItemRegistry.GetObjectTypeDefinition().GetAllData())
		{
			...
			if (category != -7 && category != -2 && StardewValley.Object.isPotentialBasicShipped(data.ItemId, data.Category, data.ObjectType))
			{
				total++;
	...
	/// <inheritdoc cref="M:StardewValley.Utility.TryGetRandom``2(System.Collections.Generic.IDictionary{``0,``1},``0@,``1@,System.Random)" />
	public static bool TryGetRandom(OverlaidDictionary dictionary, out Vector2 key, out StardewValley.Object value, Random random = null)
	{
		if (dictionary == null || dictionary.Length == 0)
		{
			key = Vector2.Zero;
		...
		KeyValuePair<Vector2, StardewValley.Object> pair = dictionary.Pairs.ElementAt(random.Next(dictionary.Length));
		key = pair.Key;
		value = pair.Value;
		return true;
	}
	...
	public static int getTrashReclamationPrice(Item i, Farmer f)
	{
		...
		if (i.canBeTrashed())
		{
			...
			if ((i is StardewValley.Object obj && !obj.bigCraftable.Value) || i is MeleeWeapon || i is Ring || i is Boots)
			{
	...
	public static void consolidateStacks(IList<Item> objects)
	{
		...
		{
			if (!(objects[i] is StardewValley.Object o))
			{
				continue;
			}
	...
	public static bool isThereAnObjectHereWhichAcceptsThisItem(GameLocation location, Item item, int x, int y)
	{
		...
		if (location.Objects.TryGetValue(tileLocation, out var obj) && obj.heldObject.Value == null && obj.performObjectDropInAction((StardewValley.Object)item, probe: true, Game1.player))
		{
			return true;
		}
		return false;
	...
	public static List<StardewValley.Object> getPurchaseAnimalStock(GameLocation location)
	{
		List<StardewValley.Object> stock = new List<StardewValley.Object>();
		foreach (KeyValuePair<string, FarmAnimalData> pair in Game1.farmAnimalData)
		{
			...
			{
				StardewValley.Object o = new StardewValley.Object("100", 1, isRecipe: false, data.PurchasePrice)
				{
					Name = pair.Key,
					Type = null
				};
	...
	public static void transferPlacedObjectsFromOneLocationToAnother(GameLocation source, GameLocation destination, Vector2? overflow_chest_position = null, GameLocation overflow_chest_location = null)
	{
		...
		foreach (Vector2 position in new List<Vector2>(source.objects.Keys))
		{
			...
			StardewValley.Object source_object = source.objects[position];
	...
	protected static void _placeOverflowChestInNearbySpace(GameLocation location, Vector2 tileLocation, StardewValley.Object o)
	{
	...
	public static Item PerformSpecialItemGrabReplacement(Item heldItem)
	{
		...
		case "(P)15":
		{
			StardewValley.Object obj = ItemRegistry.Create<StardewValley.Object>("(O)71");
	...
	public static void iterateChestsAndStorage(Action<Item> action)
	{
		ForEachLocation(delegate(GameLocation l)
		{
			...
			foreach (StardewValley.Object current in l.objects.Values)
			{
				if (current != fridge)
				{
					if (current is Chest)
	...
	public static Item addItemToThisInventoryList(Item i, IList<Item> list, int listMaxSpace = -1)
	{
		...
		while (listMaxSpace != -1 && list.Count < listMaxSpace)
		{
			if (i.Stack > i.maximumStackSize())
			{
				...
				if (i is StardewValley.Object obj2)
				{
	...
	public static bool spawnObjectAround(Vector2 tileLocation, StardewValley.Object o, GameLocation l, bool playSound = true, Action<StardewValley.Object> modifyObject = null)
	{
	...
	public static Vector2 GetNearbyValidPlacementPosition(Farmer who, GameLocation location, Item item, int x, int y)
	{
		...
		int scan_distance = 2;
		if (item is StardewValley.Object obj && obj.isPassable() && (obj.Category == -74 || obj.isSapling() || obj.Category == -19))
		{
		...
		bool is_passable = (item as StardewValley.Object)?.isPassable() ?? false;
		x = x / 64 * 64;
		y = y / 64 * 64;
	...
	public static bool tryToPlaceItem(GameLocation location, StardewValley.Object item, int x, int y)
	{
		if (item == null)
		{
			return false;
	...
	public static void recursiveObjectPlacement(StardewValley.Object o, int tileX, int tileY, double growthRate, double decay, GameLocation location, string terrainToExclude = "", int objectIndexAddRange = 0, double failChance = 0.0, int objectIndeAddRangeMultiplier = 1, List<string> itemIDVariations = null)
	{
		if (o == null)
		{
			return;
		...
		if (!Game1.random.NextBool(failChance * 2.0))
		{
			...
			{
				location.objects.Add(objectPos, new StardewValley.Object(itemId, 1)
				{
					Fragility = o.fragility.Value,
					MinutesUntilReady = o.MinutesUntilReady,
	...
	public static void recursiveFarmGrassPlacement(int tileX, int tileY, double growthRate, double decay, GameLocation farm)
	{
		if (farm.isTileLocationOpen(new Location(tileX, tileY)) && !farm.IsTileOccupiedBy(new Vector2(tileX, tileY)) && farm.doesTileHaveProperty(tileX, tileY, "Diggable", "Back") != null)
		{
			...
			{
				farm.objects.Add(new Vector2(tileX, tileY), ItemRegistry.Create<StardewValley.Object>(Game1.random.Choose("(O)674", "(O)675")));
			}
			else
			{
	...
	{
		if (item is StardewValley.Object && Game1.player.specialItems.Contains(item.ItemId))
		{
