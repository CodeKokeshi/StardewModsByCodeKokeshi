...
public abstract class IClickableMenu
{
	...
	{
		bool edibleItem = hoveredItem is StardewValley.Object hoveredObj && hoveredObj.edibility.Value != -300;
		...
		if (edibleItem && Game1.objectData.TryGetValue(hoveredItem.ItemId, out var rawData))
		{
			...
			foreach (Buff buff in StardewValley.Object.TryCreateBuffsFromData(rawData, hoveredItem.Name, hoveredItem.DisplayName, 1f, hoveredItem.ModifyItemBuffs))
			{
		...
		drawHoverText(b, hoverText, Game1.smallFont, heldItem ? 40 : 0, heldItem ? 40 : 0, moneyAmountToShowAtBottom, hoverTitle, edibleItem ? (hoveredItem as StardewValley.Object).edibility.Value : (-1), buffIcons, hoveredItem, currencySymbol, extraItemToShowIndex, extraItemToShowAmount, -1, -1, 1f, craftingIngredients, additionalCraftMaterials);
	}
	...
	public static void drawHoverText(SpriteBatch b, StringBuilder text, SpriteFont font, int xOffset = 0, int yOffset = 0, int moneyAmountToDisplayAtBottom = -1, string boldTitleText = null, int healAmountToDisplay = -1, string[] buffIconsToDisplay = null, Item hoveredItem = null, int currencySymbol = 0, string extraItemToShowIndex = null, int extraItemToShowAmount = -1, int overrideX = -1, int overrideY = -1, float alpha = 1f, CraftingRecipe craftingIngredients = null, IList<Item> additional_craft_materials = null, Texture2D boxTexture = null, Rectangle? boxSourceRect = null, Color? textColor = null, Color? textShadowColor = null, float boxScale = 1f, int boxWidthOverride = -1, int boxHeightOverride = -1)
	{
		...
		if (moneyAmountToDisplayAtBottom <= -1 && currencySymbol == 0 && hoveredItem != null && Game1.player.stats.Get("Book_PriceCatalogue") != 0 && !(hoveredItem is Furniture) && hoveredItem.CanBeLostOnDeath() && !(hoveredItem is Clothing) && !(hoveredItem is Wallpaper) && (!(hoveredItem is StardewValley.Object) || !(hoveredItem as StardewValley.Object).bigCraftable.Value) && hoveredItem.sellToStorePrice(-1L) > 0)
		{
		...
		if (hoveredItem != null)
		{
			...
			{
				if (hoveredItem is StardewValley.Object obj && obj.edibility.Value != -300 && obj.edibility.Value != 0)
				{
